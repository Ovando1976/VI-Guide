...
import { assertBookingTransition } from "@/lib/booking-workflow";
...
export async function updateServerTripStatus(params: {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (!snapshot.exists) throw new Error("Booking not found.");
    const booking = { id: snapshot.id, ...snapshot.data() } as RideBooking;
    const transition = assertBookingTransition({ booking, nextStatus: params.status, actorType: params.actorType });
    if (transition === "idempotent") return;

    const updatePayload: UpdateData<DocumentData> = {
      status: params.status,
      updatedAt: FieldValue.serverTimestamp(),
      [`statusTimestamps.${params.status}`]: FieldValue.serverTimestamp(),
    };
    if (params.status === "completed") {
      const totalFare = booking.finalFare ?? booking.quotedFare?.total ?? 0;
      updatePayload.finalFare = totalFare;
      updatePayload.settlement = { status: "pending_review", grossFare: totalFare };
    }
    transaction.update(bookingRef, updatePayload);
    if (booking.driverId && (params.status === "completed" || params.status === "cancelled")) {
      transaction.update(db.collection("drivers").doc(booking.driverId), {
        availability: "available",
        updatedAt: FieldValue.serverTimestamp(),
      });
    }
    transaction.set(db.collection("tripEvents").doc(), {
      bookingId: params.bookingId,
