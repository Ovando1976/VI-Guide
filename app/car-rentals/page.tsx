...
export default function CarRentalsPage() {
  return (
    <main className="car-rentals-page min-h-screen overflow-hidden bg-[#f5f0e6] pb-32 text-[#032f2d] md:pb-16">
      <section className="relative isolate overflow-hidden bg-[#032f2d] px-4 pb-14 pt-5 text-white sm:px-7 lg:px-10 lg:pb-20">
        ...
        <div className="mx-auto grid max-w-7xl gap-8 pb-4 pt-14 md:grid-cols-[1.15fr_.85fr] md:items-end lg:gap-10 lg:pt-24">
          <div>
            ...
            <h1 className="vi-display mt-7 max-w-4xl text-[clamp(3.7rem,8vw,7rem)] font-bold leading-[.84]">Drive with<span className="block italic text-[#73e3d9]">island confidence.</span></h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3"><Link href="#rental-start-title" className="inline-flex min-h-13 items-center gap-2 rounded-full bg-[#f5c451] px-6 py-3.5 text-[10px] font-black uppercase tracking-[.16em] text-[#032f2d]">Start with my trip <ArrowRight size={15} /></Link><Link href="#operators" className="vi-glass inline-flex min-h-13 items-center gap-2 rounded-full px-6 py-3.5 text-[10px] font-black uppercase tracking-[.16em] text-white"><CarFront size={16} className="text-[#73e3d9]" /> Compare operators</Link></div>
          </div>
          <aside className="vi-glass rounded-[30px] p-5 sm:p-7"><div className="flex gap-3"><ShieldCheck className="mt-1 h-5 w-5 shrink-0 text-[#73e3d9]" /><div><div className="vi-eyebrow text-[#f5c451]">Know before you reserve</div><h2 className="vi-display mt-3 text-3xl font-bold">USVI driving is different.</h2><p className="mt-3 text-sm font-semibold leading-6 text-white/64">Drive on the left. Confirm the deposit, insurance, age rules, after-hours return, ferry permission, and exact pickup instructions directly with the operator before relying on a reservation.</p></div></div></aside>
        </div>
      </section>
      ...
      <section id="operators" className="mx-auto max-w-7xl scroll-mt-6 px-4 pb-12 sm:px-7 lg:px-10"><div><div className="vi-eyebrow text-[#9b5d12]">Verified directory</div><h2 className="vi-display mt-3 text-4xl font-bold sm:text-6xl">Choose your pickup island.</h2><p className="mt-4 max-w-3xl text-sm font-semibold leading-7 text-slate-600">USVI Explorer verifies that these operator and location pages exist. Vehicle availability, live pricing, policies, and reservation confirmation remain the operator&apos;s responsibility. Vehicle photos are representative class imagery and do not claim to show an operator&apos;s current fleet.</p></div><div className="mt-10 space-y-12">{(["stt", "stj", "stx"] as const).map((island) => <IslandSection key={island} island={island} operators={CAR_RENTAL_OPERATORS.filter((operator) => operator.island === island)} />)}</div></section>
...
function IslandSection({ island, operators }: { island: CarRentalIsland; operators: CarRentalOperator[] }) {
  ...
  return <section><div className="relative isolate min-h-[190px] overflow-hidden rounded-[30px] bg-[#032f2d] p-6 text-white sm:p-8"><Image src={visual.image} alt={visual.alt} fill sizes="100vw" className="z-0 object-cover" /><div className="absolute inset-0 z-10 bg-[linear-gradient(90deg,rgba(3,47,45,.94),rgba(3,47,45,.42))]" /><div className="relative z-20"><div className="vi-eyebrow text-[#f5c451]">{operators.length} verified options</div><h3 className="vi-display mt-2 text-4xl font-bold">{CAR_RENTAL_ISLAND_NAMES[island]}</h3><p className="mt-3 max-w-xl text-sm font-semibold text-white/80">Compare pickup location, fleet emphasis, and operator policies before reserving.</p></div></div><div className="mt-5 grid gap-5 md:grid-cols-2">{operators.map((operator) => <OperatorCard key={operator.id} operator={operator} />)}</div></section>;
}
...
function OperatorCard({ operator }: { operator: CarRentalOperator }) {
  ...
  const conciergeHref = `/concierge?island=${operator.island}&open=true&prompt=${prompt}`;
  const rentalHref = `/car-rentals#rental-${operator.id}`;
  return <article id={`rental-${operator.id}`} className="group flex h-full scroll-mt-6 flex-col overflow-hidden rounded-[28px] border border-[#d5e4df] bg-[#fffdf8] p-5 shadow-[0_16px_45px_rgba(3,47,45,.07)] transition hover:-translate-y-0.5 hover:shadow-[0_22px_55px_rgba(3,47,45,.11)] sm:p-6"><OperatorVisual operatorId={operator.id} island={operator.island} name={operator.name} location={operator.location} /><div className="flex items-start justify-between gap-4"><div><span className="inline-flex items-center gap-1.5 rounded-full bg-[#e4f4f0] px-3 py-1.5 text-[8px] font-black uppercase tracking-[.13em] text-[#0f766e]"><BadgeCheck size={13} /> Verified directory</span><h4 className="vi-display mt-4 text-3xl font-bold">{operator.name}</h4><p className="mt-2 inline-flex items-center gap-2 text-xs font-black text-[#0f766e]"><MapPin size={15} /> {operator.location}</p></div><CarFront className="h-7 w-7 shrink-0 text-[#9b5d12]" /></div><div className="mt-5 flex flex-wrap gap-2">{operator.vehicleTypes.map((type) => <span key={type} className="rounded-full border border-[#dce7e4] bg-white px-3 py-1.5 text-[8px] font-black uppercase tracking-[.11em] text-[#49615e]">{type}</span>)}</div><div className="mt-5 space-y-2">{operator.features.map((feature) => <div key={feature} className="flex items-center gap-2 text-sm font-semibold text-slate-600"><CheckCircle2 size={15} className="text-[#0f766e]" /> {feature}</div>)}</div><p className="mt-5 text-[10px] font-semibold leading-5 text-slate-500">Source: {operator.sourceLabel}. Directory checked {operator.verifiedAt}. Live availability and policies must be confirmed with the operator.</p><div className="mt-auto grid grid-cols-2 gap-2 pt-6"><a href={operator.website} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-[#032f2d] px-4 text-[8px] font-black uppercase tracking-[.12em] text-white">Operator site <ExternalLink size={14} /></a><Link href={conciergeHref} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-[#b8e2dc] bg-[#eaf8f5] px-4 text-[8px] font-black uppercase tracking-[.12em] text-[#0f766e]">Plan rental <Sparkles size={14} /></Link><AddToJourneyButton className="col-span-2 w-full" stop={{ id: `car-rental-${operator.id}`, title: operator.name, island: operator.island, kind: "car-rental", summary: `${operator.name} car rental option in ${operator.location}. Verify live pricing, availability, insurance, deposits, age rules, pickup details, and ferry permissions directly with the operator.`, href: rentalHref, bookingHref: conciergeHref }} /></div></article>;
}

function PlanningCard({ icon: Icon, title, text }: { icon: typeof CarFront; title: string; text: string }) { return <div className="rounded-[26px] border border-[#d5e4df] bg-[#fffdf8] p-6"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#e4f4f0] text-[#0f766e]"><Icon size={20} /></span><h3 className="mt-4 text-lg font-black">{title}</h3><p className="mt-2 text-sm font-semibold leading-6 text-slate-600">{text}</p></div>; }
