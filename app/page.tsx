    ...
    <main className="min-h-screen overflow-hidden bg-[#f5f0e6] pb-[calc(12rem+env(safe-area-inset-bottom))] text-[#032f2d] sm:pb-32">
      <section className="relative isolate overflow-hidden bg-[#032f2d] px-4 pb-10 pt-5 text-white sm:px-7 lg:min-h-[850px] lg:px-10 lg:pb-14">
        ...
        <div className="mx-auto grid max-w-7xl gap-10 pb-7 pt-14 lg:grid-cols-[1.1fr_.9fr] lg:items-center lg:gap-14 lg:pb-12 lg:pt-24">
          <div>
            <div className="vi-eyebrow inline-flex items-center gap-2 rounded-full border border-[#f5c451]/30 bg-[#f5c451]/10 px-4 py-2 text-[#f9d875] backdrop-blur-xl"><SunMedium size={14} /> USVI Compass 2.0 · island travel OS</div>
            ...
            <Link href="/search" className="mt-8 flex max-w-2xl items-center gap-3 rounded-[24px] border border-white/16 bg-white/[.10] p-2.5 pl-5 shadow-[0_24px_65px_rgba(2,31,29,.28)] backdrop-blur-2xl transition hover:bg-white/[.14]"><Search size={19} className="text-[#73e3d9]" /><span className="flex-1 text-sm font-semibold text-white/58">Search beaches, stays, food, events, history, local stories…</span><span className="grid h-11 w-11 place-items-center rounded-2xl bg-white text-[#032f2d]"><ArrowRight size={18} /></span></Link>
          ...
          <aside className="vi-glass overflow-hidden rounded-[34px] p-4 sm:p-5 lg:p-6"><div className="flex items-start justify-between gap-4 px-2 pb-5"><div><div className="vi-eyebrow text-[#f5c451]">Choose your island</div><h2 className="vi-display mt-2 text-3xl font-bold text-white sm:text-4xl">Start where you are.</h2><p className="mt-2 text-sm font-semibold leading-6 text-white/55">USVI Compass keeps the island context with you across Explore, Map, Concierge, and My Trip.</p></div><Compass className="mt-1 shrink-0 text-[#73e3d9]" /></div><div className="space-y-3">{ISLANDS.map((island, index) => (<Link key={island.code} href={island.href} className="group relative flex min-h-[134px] items-end overflow-hidden rounded-[25px] border border-white/12 p-5 shadow-lg"><Image src={island.image} alt={`${island.name} in the U.S. Virgin Islands`} fill sizes="(min-width: 1024px) 40vw, 100vw" className="object-cover transition duration-700 group-hover:scale-105" /><span className="absolute inset-0 bg-[linear-gradient(90deg,rgba(2,31,29,.86),rgba(2,31,29,.32)_72%,rgba(2,31,29,.1))]" /><span className="relative flex w-full items-end justify-between gap-4"><span><span className="inline-flex rounded-full bg-[#f5c451] px-2.5 py-1 text-[8px] font-black uppercase tracking-[.14em] text-[#032f2d]">{index === 0 ? "Gateway · " : ""}{island.code}</span><span className="mt-2 block text-xl font-black text-white">{island.name}</span><span className="mt-1 block text-xs font-semibold text-white/68">{island.line}</span></span><span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl border border-white/16 bg-white/[.10] text-white backdrop-blur transition group-hover:bg-[#f5c451] group-hover:text-[#032f2d]"><ArrowRight size={17} /></span></span></Link>))}</div></aside>
        </div>
        <div className="mx-auto max-w-7xl"><div className="vi-glass grid grid-cols-2 gap-2 rounded-[30px] p-2.5 sm:grid-cols-3 lg:grid-cols-8">{QUICK.map(({ label, detail, href, image, alt, icon: Icon }) => (<Link key={label} href={href} className="group relative flex min-h-[112px] items-end overflow-hidden rounded-[21px] border border-white/10 bg-[#032f2d] p-3 shadow-[0_12px_32px_rgba(2,31,29,.16)] transition hover:-translate-y-0.5 hover:border-[#f5c451]/55"><Image src={image} alt={alt} fill sizes="(min-width: 1024px) 16vw, (min-width: 640px) 33vw, 50vw" className="object-cover transition duration-700 group-hover:scale-105" /><span className="absolute inset-0 bg-[linear-gradient(180deg,rgba(2,31,29,.08),rgba(2,31,29,.36)_48%,rgba(2,31,29,.93)_100%)]" /><span className="relative flex w-full items-end gap-2.5"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white/92 text-[#0f766e] shadow-lg backdrop-blur transition group-hover:bg-[#f5c451] group-hover:text-[#032f2d]"><Icon size={17} /></span><span className="min-w-0 pb-0.5"><span className="block truncate text-xs font-black text-white">{label}</span><span className="mt-0.5 block truncate text-[9px] font-bold text-white/62">{detail}</span></span></span></Link>))}</div></div>
      </section>
