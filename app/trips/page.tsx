          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span
                  className={`rounded-full px-3 py-2 text-[8px] font-black uppercase tracking-[.13em] ${status.className}`}
                >
                  {status.label}
                </span>
              </div>
...
function stayStatus(value: unknown) {
  const normalized = typeof value === "string" ? value.trim().toLowerCase() : "";
  return (
    STAY_STATUS[normalized] ?? {
      label: normalized
