    ...
    <main className="stay-detail min-h-screen bg-[#f8f4ea] pb-36 text-[#043331]">
      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8 lg:py-8">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="text-[10px] font-black uppercase tracking-[.2em] text-slate-500">
            {islandName} · {item.location ?? item.category}
          </div>
        </div>

        <section className="relative overflow-hidden rounded-[34px] bg-[#043331] shadow-[0_30px_80px_rgba(4,51,49,.2)]">
          <div
            className="relative min-h-[440px] bg-cover bg-center sm:min-h-[520px]"
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9 lg:p-11">
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-full bg-[#f5b942] px-3 py-2 text-[9px] font-black uppercase tracking-[.2em] text-[#043331]">
                  Verified stay
                </span>
                <span className="rounded-full border border-white/25 bg-black/20 px-3 py-2 text-[9px] font-black uppercase tracking-[.2em] backdrop-blur">
                  {item.category}
                </span>
              </div>
              <h1 className="mt-4 max-w-4xl text-[clamp(2.6rem,6vw,5.4rem)] font-black leading-[.92] tracking-[-.06em]">
                {item.name}
              </h1>
              <div className="mt-5 flex flex-wrap gap-x-6 gap-y-2 text-sm font-semibold text-white/80">
                {item.location ? (
                  <span className="inline-flex items-center gap-2">
                    <MapPin size={16} /> {item.location}, {islandName}
                  </span>
                ) : null}
                <span className="inline-flex items-center gap-2">
                  <BadgeCheck size={16} className="text-[#7ce0d4]" /> Reviewed {item.verifiedAt}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-7">
              ...
              <div className="grid gap-8 md:grid-cols-2">
                ...
                <div>
                  ...
                  <p className="mt-5 text-sm font-semibold leading-6 text-slate-600">
                    Verified using {item.sourceLabel}. VI Guide distinguishes catalog verification from live room availability, rates, and reservation confirmation.
                  </p>
                  {item.sourceUrl ? (
                    <a
                      href={item.sourceUrl}
            ...
            <section className="overflow-hidden rounded-[30px] bg-[#043331] p-7 text-white sm:p-8">
              ...
              <p className="mt-4 max-w-2xl text-sm font-semibold leading-6 text-white/65">
                Ask the concierge for a grounded plan built around this property, including arrival timing, nearby beaches, dinner options, ferry connections, and a return ride.
              </p>
            </section>
          </div>
