...
export async function GET(request: NextRequest) {
  ...
  const islandCode: IslandCode = requested === "stj" || requested === "stx" ? requested : "stt";
  const island = ISLANDS[islandCode];
  const headers = { Accept: "application/geo+json", "User-Agent": "USVI Explorer beach intelligence (https://www.usvi-explorer.com)" };

  try {
    const pointResponse = await fetch(`https://api.weather.gov/points/${island.latitude},${island.longitude}`, { headers, next: { revalidate: 900 } });
    if (!pointResponse.ok) throw new Error(`NWS point lookup failed with ${pointResponse.status}.`);
    const point = (await pointResponse.json()) as { properties?: { forecastHourly?: string; forecastZone?: string } };
    const forecastUrl = point.properties?.forecastHourly;
    if (!forecastUrl) throw new Error("NWS hourly forecast URL is unavailable.");
    const forecastResponse = await fetch(forecastUrl, { headers, next: { revalidate: 900 } });
    if (!forecastResponse.ok) throw new Error(`NWS hourly forecast failed with ${forecastResponse.status}.`);
