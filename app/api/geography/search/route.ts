...
export async function GET(request: NextRequest) {
  try {
    ...
    if (island !== "ALL") {
      entries = entries.filter((entry) => entry.island === island);
    }

    if (type !== "all") {
      entries = entries.filter((entry) => {
        const name = (entry.canonicalName || "").trim().toUpperCase();

        if (!name) return false;
        if (name.includes("GEOGRAPHICDICTIONARY")) return false;
        if (name.includes("COASTANDGEODETIC")) return false;
        if (name.includes("U.S.COAST")) return false;
        if (name.includes("VIRGINISLANDS")) return false;
    ...
    const results = entries
      .map((entry) => {
        ...
        const phraseMatch =
          canonical.includes(nq) ||
          aliases.some((alias) => normalizeText(alias).includes(nq));
