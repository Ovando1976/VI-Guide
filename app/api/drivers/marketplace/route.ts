  ...
  try {
    ...
    if (!driverSnapshot.exists) {
      return NextResponse.json({
        ...
        candidateCount: 0,
        readinessIssue:
          "Your account is not linked to an active driver profile yet.",
      });
    }
    ...
    if (!vehicleId || !associationId) {
      return NextResponse.json({
        ...
        candidateCount: 0,
        readinessIssue:
          "Link an active association vehicle before taking paid ride requests.",
      });
    }

    const [vehicleSnapshot, associationSnapshot, bookingsSnapshot] =
      await Promise.all([
    ...
    if (!vehicleSnapshot.exists || !associationSnapshot.exists) {
      return NextResponse.json({
        ...
        readinessIssue:
          "Your linked taxi fleet records are incomplete. Ask dispatch to review your vehicle and association.",
      });
    }
    ...
    for (const bookingSnapshot of bookingsSnapshot.docs) {
      ...
      try {
        ...
        // The same authoritative eligibility check runs again transactionally on accept.
        // Incompatible requests simply stay out of this driver's marketplace.
      }
    }
    ...
    return NextResponse.json(
      { error: "Unable to load compatible ride requests." },
      { status: 500 },
    );
  }
}
