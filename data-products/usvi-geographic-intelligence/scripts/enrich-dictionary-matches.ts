...
function asMatch(entry: DictionaryEstate, reason: string): DictionaryMatch {
  return {
    id: entry.id,
    name: entry.name ?? entry.rawName,
    normalizedName: entry.normalizedName ?? normalize(entry.name ?? entry.rawName),
...
async function main() {
  const [estateRaw, dictionaryRaw, manualRaw] = await Promise.all([
    readFile(ESTATES_PATH, "utf8"),
    readFile(DICTIONARY_PATH, "utf8"),
    readFile(MANUAL_LINKS_PATH, "utf8").catch(() => "[]"),
  ]);
  ...
  const dictionaryById = new Map(dictionary.filter((entry) => entry.id).map((entry) => [entry.id as string, entry]));
  const manualByGeoid = new Map<string, DictionaryEstate[]>();
  for (const link of manualLinks) {
    const entry = dictionaryById.get(link.dictionaryId);
    if (!entry) throw new Error(`Manual dictionary link references missing dictionary id: ${link.dictionaryId}`);
    const current = manualByGeoid.get(link.modernGeoid) ?? [];
    current.push(entry);
    manualByGeoid.set(link.modernGeoid, current);
  }
  ...
  for (const estate of estates) {
    ...
    if (manualMatches.length) {
      estate.dictionaryMatches = manualMatches.map((entry) => asMatch(entry, "manual-geoid-link"));
      manual++;
      continue;
    }
    ...
    if (exactCandidates.length === 1) {
      estate.dictionaryMatches = [asMatch(exactCandidates[0], "exact-same-island-name")];
      exact++;
      continue;
    }
    ...
    if (candidates.length === 1) {
      estate.dictionaryMatches = [asMatch(candidates[0], "verified-same-island-alias")];
      alias++;
      continue;
    }
    ...
    if (candidates.length > 1) {
      ...
      for (const entry of candidates) {
        review.push({
          modernEstate: { geoid: estate.geoid, baseName: estate.baseName, island: estate.island },
          dictionaryEntry: { id: entry.id, name: entry.name ?? entry.rawName, normalizedName: entry.normalizedName },
          reason: "ambiguous-same-island-name-or-alias",
  ...
  await Promise.all([
    writeFile(ESTATES_PATH, JSON.stringify(estates, null, 2) + "\n", "utf8"),
    writeFile(REVIEW_PATH, JSON.stringify(review, null, 2) + "\n", "utf8"),
  ]);
...
main().catch((error) => {
  console.error("Estate dictionary enrichment failed:", error);
  process.exit(1);
});
