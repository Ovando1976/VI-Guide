...
type ReviewCandidate = {
  ...
  dictionaryEntry?: { name?: string; normalizedName?: string };
  reason?: string;
  score?: number;
};
...
type AuditRow = {
  ...
  match_type: AuditStatus;
  island: IslandCode;
  coordinate_status: "OK" | "MISSING" | "OUT_OF_BOUNDS";
  geometry_status: "OK" | "MISSING";
  provenance_status: "OK" | "MISSING";
...
function classify(
  ...
  const key = `${estate.island}:${normalize(estate.baseName)}`;
  if (duplicateKeys.has(key)) {
    return { status: "DUPLICATE", reason: "Multiple modern estates share the same normalized island/name key.", confidence: "low" };
  }
...
async function main() {
  ...
  const duplicateKeys = new Set(
    [...identityCounts.entries()].filter(([, count]) => count > 1).map(([key]) => key),
  );

  const reviewByGeoid = new Map<string, ReviewCandidate[]>();
  for (const review of reviews) {
    const geoid = String(review.modernEstate?.geoid ?? "").trim();
    if (!geoid) continue;
    const current = reviewByGeoid.get(geoid) ?? [];
    current.push(review);
    reviewByGeoid.set(geoid, current);
  }
  ...
  const audit: AuditRow[] = estates
    .map((estate) => {
      ...
      return {
        ...
        canonical_name: estate.baseName,
        source_name: uniqueSorted(estate.sources ?? []).join(" | "),
        existing_name: estate.fullName ?? estate.baseName,
        match_type: classification.status,
        island: estate.island,
        coordinate_status: pointStatus(estate.centroid),
        geometry_status: estate.geometry ? "OK" : "MISSING",
        provenance_status: (estate.sources?.length ?? 0) > 0 ? "OK" : "MISSING",
        confidence: classification.confidence,
        review_reason: classification.reason,
      };
    })
    .sort((a, b) => a.island.localeCompare(b.island) || a.canonical_name.localeCompare(b.canonical_name));

  const canonical = estates
    .map((estate) => ({
      canonical_id: estate.geoid,
      geoid: estate.geoid,
      estate_code: estate.estateCode ?? "",
      name: estate.baseName,
      full_name: estate.fullName ?? estate.baseName,
      normalized_name: normalize(estate.baseName),
      aliases: uniqueSorted(estate.aliases ?? []),
      island: estate.island,
      county: estate.county ?? "",
      centroid_lat: estate.centroid?.lat ?? null,
      centroid_lng: estate.centroid?.lng ?? null,
      internal_point_lat: estate.internalPoint?.lat ?? null,
      internal_point_lng: estate.internalPoint?.lng ?? null,
      sources: uniqueSorted(estate.sources ?? []),
      historical_notes: uniqueSorted(estate.historicalNotes ?? []),
      dictionary_names: uniqueSorted((estate.dictionaryMatches ?? []).flatMap((match) => [match.name, match.normalizedName])),
      geometry_type: estate.geometry?.type ?? "",
    }))
    .sort((a, b) => a.island.localeCompare(b.island) || a.name.localeCompare(b.name));

  const features: GeoJSON.Feature[] = estates
    .filter((estate) => Boolean(estate.geometry))
    .map((estate) => ({
      type: "Feature",
      id: estate.geoid,
      geometry: estate.geometry as GeoJSON.Polygon | GeoJSON.MultiPolygon,
      properties: {
        canonical_id: estate.geoid,
        geoid: estate.geoid,
        estate_code: estate.estateCode ?? "",
  ...
  const summary = audit.reduce<Record<string, number>>((counts, row) => {
    counts.total = (counts.total ?? 0) + 1;
    counts[row.match_type] = (counts[row.match_type] ?? 0) + 1;
    if (row.coordinate_status !== "OK") counts.coordinate_issues = (counts.coordinate_issues ?? 0) + 1;
    if (row.geometry_status !== "OK") counts.geometry_issues = (counts.geometry_issues ?? 0) + 1;
    if (row.provenance_status !== "OK") counts.provenance_issues = (counts.provenance_issues ?? 0) + 1;
    return counts;
  }, {});
  ...
  await Promise.all([
    writeFile(path.join(CSV_DIR, "usvi-estates.csv"), toCsv(canonical), "utf8"),
    writeFile(path.join(CSV_DIR, "audit-estates.csv"), toCsv(audit), "utf8"),
    writeFile(
      path.join(GEOJSON_DIR, "usvi-estates.geojson"),
...
main().catch((error) => {
  console.error("Estate Intelligence build failed:", error);
  process.exit(1);
});
