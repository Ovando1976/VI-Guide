...
if (!targetId || !nextStatus || !allowed.has(nextStatus)) {
  console.error("Usage: npm run history:graph:rigsarkivet:set-status -- <target-id> <status>");
  process.exit(1);
}

const file = path.resolve("src/data/historyGraph/extractionTargets/rigsarkivetExtractionTargets.ts");
