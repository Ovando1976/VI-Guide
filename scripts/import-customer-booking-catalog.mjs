...
const records = rows.map((row) => {
  ...
  return {
    ...
    image: item.image.trim() || "/images/accommodations/_pending/hero.svg",
    imageAlt: item.imageAlt?.trim() || `${item.businessName.trim()} accommodation image`,
    imageSourceName: item.imageSourceName?.trim() || "Image pending verification",
    imageSourceUrl: item.imageSourceUrl?.trim() || undefined,
    imageStatus: item.imageStatus?.trim() || "needs_image",
