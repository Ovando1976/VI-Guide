...
assert.equal(bookingKindForProposalStop({ kind: "hotel" }), "accommodation");
...
assert.equal(bookingKindForProposalStop({ kind: "cultural experience" }), "experience");
...
assert.equal(hotelUrl.searchParams.get("listingHref"), "/accommodations/example-hotel");
...
assert.equal(new URL(unsafeHref, "https://vi-guide.local").searchParams.has("listingHref"), false);

assert.equal(buildTravelAdvisorBookingHref({
  shareId,
  date: "2026-09-13",
  stop: {
    id: "beach_1",
...
assert.equal(proposalBookingEmailMatches(" Traveler@Example.com ", "traveler@example.com"), true);
assert.equal(proposalBookingEmailMatches("traveler@example.com", "other@example.com"), false);
