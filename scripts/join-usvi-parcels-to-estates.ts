...
function normalizeEstates(records: EstateRecord[]): EstateCandidate[] {
  ...
  for (const record of records) {
    ...
    const feature: Feature<PolygonGeometry> = { type: "Feature", properties: {}, geometry: record.geometry };
    estates.push({
      geoid,
      name,
...
async function main() {
  ...
  for (const parcel of parcelCollection.features) {
    ...
    joined.push({
      ...parcelIdentity,
      estate: { geoid: estate.geoid, name: estate.name, fullName: estate.fullName, island: estate.island },
      join: { method: "turf-point-on-feature-within-canonical-estate-polygon", status: "matched" },
      provenance: {
  ...
  if (!releaseReady) {
    console.error(`Property overlay release gate remains closed: ${unmatched.length} unmatched, ${ambiguous.length} ambiguous.`);
    process.exitCode = 2;
  }
}
