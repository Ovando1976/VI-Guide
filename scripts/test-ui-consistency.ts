...
const territoryState = source("hooks/use-territory-state.ts");
const livingMapBridge = source("components/workspace/living-map-bridge.tsx");
const estateMap = source("components/estate-map.tsx");
...
assert.ok(territoryState.indexOf('const id = boundedText(params.get("place")') < territoryState.indexOf('const estate = boundedText(params.get("estate")'),"Living Map must prefer complete place context while retaining estate pricing context");assert.match(territoryState,/"placeSlug"/);assert.match(territoryState,/"placeHref"/);assert.match(territoryState,/const preservesIncomingContext = sourcePlaceId === place\.id/);assert.match(territoryState,/preservesIncomingContext \? \(contextEstate \?\? null\) : null/);assert.match(livingMapBridge,/const SELECTION_KEYS = \[[\s\S]*?"placeSlug",[\s\S]*?"placeHref"/);assert.match(livingMapBridge,/lastApplied\.current === null && hasExplicitSelection/);assert.match(livingMapBridge,/lastApplied\.current = focus\.issuedAt;\s+return;/);
assert.match(estateMap,/useEffect\(\(\) => \{\s+if \(focusedPlaceId\) return;\s+setSelectedPlaceId\(null\);\s+onSelectPlace\?\.\(null\);/);
