...
function stripLeadingNoise(value: string) {
  return normalizeWhitespace(
    value.replace(/^[0-9&.,;:()\- ]+/, "").replace(/^(and|the)\s+/i, "")
  );
}
...
function hasTooManyWeirdCharacters(value: string) {
  const weird = (value.match(/[^A-Za-z0-9 .,'()\-]/g) || []).length;
  return weird >= 4;
}
...
function looksLikeHeaderOrJunkName(value: string) {
  ...
  if (hasTooManyWeirdCharacters(v)) return true;
  if ((v.match(/[.;:()]/g) || []).length >= 4) return true;
  if (/[0-9]{3,}/.test(v)) return true;
  if (/^[^A-Za-z]+$/.test(v)) return true;
  if (tooManyCaps(v)) return true;
  if (BLOCKED_NAME_FRAGMENTS.some((x) => upper.includes(x))) return true;
  if (BAD_PREFIXES.some((x) => upper.startsWith(x))) return true;
...
function looksLikeMergedParagraph(value: string) {
  ...
  return (
    compact.length > 45 &&
    !value.includes(" ") &&
    /[a-z]{8,}[A-Z][a-z]{5,}/.test(value)
  );
}

function inferIsland(entry: GeographicDictionaryEntry) {
  const text = `${entry.canonicalName} ${entry.shortDescription || ""} ${
    entry.description || ""
  } ${entry.rawText || ""}`;

  const upper = text.toUpperCase();

  const hasSTT = /\bST\.\s*THOMAS\b|\bST THOMAS\b/.test(upper);
  const hasSTJ = /\bST\.\s*JOHN\b|\bST JOHN\b/.test(upper);
  const hasSTX = /\bST\.\s*CROIX\b|\bST CROIX\b/.test(upper);

  if (hasSTT && !hasSTJ && !hasSTX) return "STT";
  if (hasSTJ && !hasSTT && !hasSTX) return "STJ";
  if (hasSTX && !hasSTT && !hasSTJ) return "STX";

  return entry.island && ALLOWED_ISLANDS.has(entry.island)
    ? entry.island
    : "UNKNOWN";
}
...
function inferFeatureType(entry: GeographicDictionaryEntry) {
  const haystack = normalizeGeoText(
    `${entry.canonicalName} ${entry.shortDescription || ""} ${
      entry.description || ""
    }`
  );

  for (const [type, keywords] of Object.entries(FEATURE_TYPE_KEYWORDS)) {
    if (keywords.some((k) => haystack.includes(k.trim()))) {
      return type === "harbor_feature" ? "harbor" : type;
    }
  }
...
function dedupeStrings(values: string[] | undefined) {
  ...
  for (const value of values || []) {
    ...
    const key = normalizeGeoText(cleaned);
...
function buildSearchTokens(entry: GeographicDictionaryEntry) {
  ...
  const add = (value?: string) => {
    const cleaned = normalizeGeoText(value || "");
    if (!cleaned) return;
  ...
  add(entry.normalizedName);
  add(entry.shortDescription);
...
function scoreEntry(entry: GeographicDictionaryEntry) {
  ...
  if (entry.parseConfidence != null) {
    score += Math.round((entry.parseConfidence - 1) * 20);
  }
...
function cleanEntry(entry: GeographicDictionaryEntry) {
  ...
  cleaned.parseConfidence = confidence;
  ...
  if (looksLikeMergedParagraph(cleaned.canonicalName))
    warnings.add("Possible merged OCR text");
...
async function main() {
  ...
  if (dropped.length) {
    ...
    for (const item of dropped.slice(0, 20)) {
      console.log(`- ${item.id}: ${item.canonicalName}`);
    }
  }
}

main().catch((error) => {
  console.error("Geographic dictionary cleanup failed:", error);
  process.exit(1);
});
