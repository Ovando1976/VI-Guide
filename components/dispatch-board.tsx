...
export function DispatchBoard() {
  ...
  const bookingHasOfficialFare =
    selectedBooking?.quotedFare?.quoteStatus === "official";
  ...
  async function moveStatus(bookingId: string, status: StatusKey) {
    if (!selectedBooking || !bookingIsPaid || !bookingHasOfficialFare) return;

    let message = dispatchMessage(status);
    if (status === "cancelled") {
      const reason = window.prompt(
        "Enter the operational cancellation reason (required for the audit trail):"
      );
      if (reason === null) return;
      if (reason.trim().length < 8) {
        window.alert("Please enter a specific cancellation reason.");
        return;
      }
      message = `Dispatch cancelled the trip: ${reason.trim()}`;
    }

    if (
      status === "in_progress" &&
      !window.confirm("Confirm the rider is aboard before starting this trip?")
    ) {
      return;
    }
    if (
      status === "completed" &&
      !window.confirm("Confirm the rider arrived before completing this trip?")
    ) {
      return;
    }
        ...
        <section className="grid gap-5 xl:grid-cols-[1.24fr_0.76fr]">
          ...
          <div className="space-y-6">
            ...
            >
              {selectedBooking ? (
                <div className="space-y-6">
                  {!bookingIsPaid || !bookingHasOfficialFare ? (
                    <div className="rounded-[24px] border border-amber-200 bg-amber-50 p-4">
                      ...
                      <div className="mt-2 text-sm font-semibold text-amber-800">
                        ...
                        changes require both cleared payment and an official tariff
                        quote. Provisional records remain in rate review.
                      </div>
                    </div>
                  ) : (
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <OpsPill
                      ...
                      tone={bookingHasOfficialFare ? "emerald" : "amber"}
                    />
                  </div>
