...
export function CommerceBookingForm() {
      ...
      <main className="min-h-screen bg-[#f8f4ea] px-4 py-10 text-[#043331] sm:px-6">
        <section className="mx-auto max-w-2xl rounded-[36px] border border-emerald-200 bg-white p-7 text-center shadow-[0_30px_80px_rgba(4,51,49,.12)] sm:p-10">
          ...
          <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
            <Link
              href="/planner"
              className="inline-flex min-h-12 items-center justify-center rounded-full bg-[#043331] px-6 text-[10px] font-black uppercase tracking-[.16em] text-white"
            >
              Open my trip
            </Link>
            <Link
              href="/concierge"
              className="inline-flex min-h-12 items-center justify-center rounded-full border border-slate-200 bg-white px-6 text-[10px] font-black uppercase tracking-[.16em]"
            >
              Ask concierge
            </Link>
          </div>
            ...
            <div className="bg-[radial-gradient(circle_at_top_left,rgba(245,196,81,.25),transparent_36%),linear-gradient(145deg,#043331,#075e58)] p-7 sm:p-10">
              ...
              <div className="mt-8 space-y-4 text-sm font-semibold leading-6 text-white/68">
                <div className="flex gap-3">
                  ...
                  <span>This flow records a real booking request without presenting unverified availability or pricing.</span>
                </div>
                <div className="flex gap-3">
                  <Mail className="mt-0.5 h-5 w-5 shrink-0 text-[#7ce0d4]" />
                  <span>Your contact information is used to follow up on this request.</span>
                </div>
              </div>
            </div>

            <form onSubmit={submit} className="bg-white p-6 text-[#043331] sm:p-8 lg:p-10">
              <div className="grid gap-5 sm:grid-cols-2">
                <Field label={kind === "accommodation" ? "Check-in" : "Date"} icon={CalendarDays}>
                  <input
                    required
                    type="date"
                    min={today}
                ...
                <Field label="Adults" icon={Users}>
                  <input
                    ...
                    max={20}
                    value={adults}
                    onChange={(event) => setAdults(clamp(Number(event.target.value), 1, 20))}
                    className="field-input"
                  />
                </Field>
                ...
                <Field label="Children" icon={Users}>
                  <input
                    ...
                    max={20}
                    value={children}
                    onChange={(event) => setChildren(clamp(Number(event.target.value), 0, 20))}
                    className="field-input"
                  />
                </Field>
...
function normalizeKind(value: string | null): CommerceBookingKind {
  return value === "tour" || value === "experience" ? value : "accommodation";
}
