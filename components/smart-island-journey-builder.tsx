...
import { buildIslandJourneyMapStops } from "@/lib/island-journey-map";
...
export function SmartIslandJourneyBuilder({
  ...
  catalogPlaces?: JourneyPlace[];
}) {
  const router = useRouter();
  const places = useMemo(
    () => [...JOURNEY_PLACES, ...catalogPlaces],
    [catalogPlaces],
  );
  ...
  const sameIsland = Boolean(
    origin && destination && origin.island === destination.island,
  );
  ...
  function persistPlan(planToSave: SmartJourneyPlan) {
    const journey = createJourneyPlan(
      planToSave.origin.island,
      `${planToSave.origin.label} → ${planToSave.destination.label}`,
    );
    const stops = buildIslandJourneyMapStops(planToSave, journey.id);
  ...
  function savePlan(planToSave: SmartJourneyPlan) {
    persistPlan(planToSave);
    router.push("/trips");
  }

  function openLivingMap(planToSave: SmartJourneyPlan) {
    const journeyId = persistPlan(planToSave);
    router.push(`/map/journey?trip=${encodeURIComponent(journeyId)}`);
  }
  ...
  return (
    <section className="rounded-[32px] border border-[#0b5b57]/15 bg-white p-5 shadow-[0_22px_70px_rgba(4,51,49,.08)] md:p-8">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[.22em] text-[#b7861f]">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        <Field label="From">
          <select
            ...
            className="w-full rounded-2xl border border-slate-200 bg-[#f8f4ea] px-4 py-3 text-sm font-bold text-[#043331]"
          >
        ...
        <Field label="Date">
          <div className="relative">
            <CalendarDays className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-[#0b817b]" />
            <input
              type="date"
              value={travelDate}
              onChange={(event) => setTravelDate(event.target.value)}
              className="w-full rounded-2xl border border-slate-200 bg-[#f8f4ea] py-3 pl-10 pr-3 text-sm font-bold text-[#043331]"
            />
          </div>
        </Field>
        <Field label="Timing">
          <select
            value={timeMode}
            onChange={(event) =>
              setTimeMode(event.target.value as JourneyTimeMode)
            }
            className="w-full rounded-2xl border border-slate-200 bg-[#f8f4ea] px-4 py-3 text-sm font-bold text-[#043331]"
          >
            <option value="departAfter">Leave after</option>
            <option value="arriveBy">Arrive by</option>
          </select>
        </Field>
        <Field label="Time">
          <div className="relative">
            <Clock3 className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-[#0b817b]" />
            <input
              type="time"
              value={requestedTime}
      ...
      {routing ? (
        <div className="mt-6 flex items-center gap-3 rounded-[24px] border border-teal-100 bg-teal-50 p-5 text-sm font-bold text-teal-900">
          <LoaderCircle className="h-5 w-5 animate-spin" />
          Calculating the road connections to the best ferry terminals…
        </div>
      ) : plan ? (
        <div className="mt-6 overflow-hidden rounded-[28px] border border-[#0b5b57]/12 bg-[#f8f4ea]">
          <div className="grid gap-4 border-b border-[#0b5b57]/10 p-5 md:grid-cols-4 md:p-6">
            <Stat label="Leave origin" value={plan.leaveOriginTime} />
            <Stat label="Ferry" value={plan.ferryDepartureTime} />
            <Stat label="Arrive" value={plan.destinationArrivalTime} />
            <Stat label="Journey time" value={`${plan.totalMinutes} min`} />
          </div>
          {plan.warning ? (
            <div className="flex gap-2 border-b border-amber-200 bg-amber-50 px-5 py-3 text-xs font-bold text-amber-900">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#e5f4f1] text-[#08746f]">
                  ...
                  <h3 className="mt-4 text-lg font-black text-[#043331]">
                    {leg.from}{" "}
                    <ArrowRight className="mx-1 inline h-4 w-4 text-[#b7861f]" />{" "}
                    {leg.to}
                  </h3>
                  ...
                  {leg.mobilityHref ? (
                    <Link
                      href={leg.mobilityHref}
                      className="mt-4 inline-flex rounded-full border border-[#0b817b]/20 bg-white px-4 py-2 text-[10px] font-black uppercase tracking-[.12em] text-[#08746f]"
                    >
                      Open ride
                    </Link>
                  ) : null}
          ...
          <div className="flex flex-wrap gap-3 bg-[#043f3b] p-5 md:px-6">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-full bg-[#f3c44e] px-5 py-3 text-xs font-black uppercase tracking-[.13em] text-[#043331]"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded-full border border-white/25 px-5 py-3 text-xs font-black uppercase tracking-[.13em] text-white"
            >
              Verify ferry source
            </a>
          </div>
        ...
        <div className="mt-6 rounded-[24px] border border-dashed border-[#0b5b57]/20 bg-[#f8f4ea] p-6">
          <h3 className="font-black text-[#043331]">
            {sameIsland
              ? "No ferry needed for this pair."
              : "No supported ferry connection fits these selections."}
          </h3>
          ...
          <div className="mt-4 flex gap-3">
            <Link
              href="/mobility"
              className="rounded-full bg-[#043f3b] px-4 py-2.5 text-[10px] font-black uppercase tracking-[.12em] text-white"
            >
              Open Mobility
            </Link>
            <Link
              href="/concierge?prompt=Help%20me%20coordinate%20an%20inter-island%20connection"
              className="rounded-full border border-[#043f3b]/20 px-4 py-2.5 text-[10px] font-black uppercase tracking-[.12em] text-[#043f3b]"
            >
              Ask Concierge
            </Link>
          </div>
...
async function resolveTerminalTransfers(
  place: JourneyPlace,
  direction: "origin" | "destination",
) {
  if (
    Object.keys(place.terminalTransfers).length ||
    place.lat == null ||
    place.lng == null
  ) {
    return place;
  ...
  await Promise.all(
    ferryPortsForIsland(place.island).map(async (port) => {
      ...
      try {
        ...
        if (
          ...
          terminalTransfers[port] = Math.max(
            3,
            Math.ceil(result.durationSeconds / 60),
          );
        }
      } catch {
        // A missing road estimate removes only this terminal candidate.
      }
    }),
  );
...
function renderPlaceOptions(places: JourneyPlace[]) {
  ...
  return islands.map(([island, label]) => (
    <optgroup key={island} label={label}>
      {places
        .filter((place) => place.island === island)
        .map((place) => (
...
function Field({
  ...
  return (
    <label className="block">
      <span className="mb-2 block text-[10px] font-black uppercase tracking-[.14em] text-slate-500">
        {label}
      </span>
      {children}
    </label>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="mt-1 text-xl font-black text-[#043331]">{value}</div>
    </div>
  );
}
