...
import { isActionable, isPermanentlyClosed, isTemporarilyClosed } from "@/lib/catalog-status";
...
export function DirectoryDetailScreen({ slug, kind }: Props) {
  ...
  const permanentlyClosed = isPermanentlyClosed(item);
  const temporarilyClosed = isTemporarilyClosed(item);
  ...
  return (
    <main className="min-h-screen bg-[#f8f4ea] px-4 py-6 pb-32 text-[#043331] sm:px-6 lg:py-10">
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        {!actionable ? (
          <section className={`rounded-[24px] border p-5 ${permanentlyClosed ? "border-rose-300 bg-rose-50 text-rose-950" : "border-amber-300 bg-amber-50 text-amber-950"}`}>
            <div className="text-xs font-black uppercase tracking-[.18em]">{permanentlyClosed ? "Reported permanently closed" : item.operatingStatus === "seasonal" ? "Seasonal access restrictions" : "Reported temporarily closed"}</div>
            <p className="mt-2 text-sm font-semibold leading-6">This status comes from the catalog evidence linked below. Planning and ride actions are disabled until the location is verified as available.</p>
          </section>
        ) : null}
          ...
          <div className="grid lg:grid-cols-[1.25fr_.75fr]">
            ...
            <div className="flex flex-col justify-between bg-[linear-gradient(145deg,#043331_0%,#0b5d5b_62%,#14b8a6_100%)] p-7 text-white sm:p-10">
              ...
              <div className="mt-10 grid gap-3 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                ...
                <ActionLink href={rideHref} icon={Navigation} label={actionable ? "Ride here" : "Ride unavailable"} accent disabled={!actionable} />
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-7 lg:grid-cols-[1fr_380px]">
          ...
          <aside className="space-y-5 lg:sticky lg:top-6 lg:self-start">
            <section className="rounded-[28px] border border-amber-200 bg-white p-5 shadow-sm">
              <div className="text-[10px] font-black uppercase tracking-[.2em] text-amber-600">
                Make it part of the trip
              </div>
              <p className="mt-2 text-sm font-semibold leading-6 text-slate-600">
                Save it in one click. VI Guide will suggest a practical day and time.
              </p>
              <AddToTripButton
                className="mt-4"
                disabled={!actionable}
                disabledReason={item.operatingStatus === "seasonal" ? "Confirm seasonal access" : temporarilyClosed ? "Temporarily closed" : "Location closed"}
                item={{
                  id: item.id,
                  slug: item.slug,
                  name: item.name,
            ...
            <Panel eyebrow="Location" title="Territory details">
              <div className="grid gap-3">
                ...
                {item.reservationUrl && actionable ? <Fact icon={ExternalLink} label="Reservations" value="Reserve on the official site" href={item.reservationUrl} external /> : null}
              </div>
...
function VerificationPanel({ item }: { item: DirectoryItem }) {
  const verified = item.verificationLevel && item.verificationLevel !== "unverified";
  const status = item.operatingStatus ?? "unconfirmed";
  return (
    <section className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[.18em] text-slate-500">
        <ShieldCheck className={`h-4 w-4 ${verified ? "text-emerald-600" : "text-amber-600"}`} />
        Catalog confidence
      </div>
      <div className="mt-3 text-sm font-black text-[#043331]">
        {status === "verified-operating" ? "Operating status verified" : status.replaceAll("-", " ")}
      </div>
      <p className="mt-2 text-xs font-semibold leading-5 text-slate-500">
        {item.verifiedAt ? `Details checked ${item.verifiedAt}.` : "Live operating details have not yet been independently confirmed."}
        {item.hoursNote ? ` ${item.hoursNote}` : " Confirm hours directly before traveling."}
      </p>
      {item.sourceUrl ? (
        <a href={item.sourceUrl} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-2 text-xs font-black text-teal-700">
          <ExternalLink className="h-3.5 w-3.5" /> {item.sourceLabel ?? "View verification source"}
        </a>
      ) : null}
    </section>
  );
...
function ActionLink({ href, icon: Icon, label, accent = false, disabled = false }: { href: string; icon: typeof Map; label: string; accent?: boolean; disabled?: boolean }) {
  const classes = `inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-[11px] font-black uppercase tracking-[.17em] ${disabled ? "cursor-not-allowed border border-white/15 bg-white/5 text-white/45" : accent ? "bg-[#fbbf24] text-[#043331]" : "border border-white/30 bg-white/10 text-white"}`;
  return disabled ? <span aria-disabled="true" className={classes}><Icon className="h-4 w-4" />{label}</span> : <Link href={href} className={classes}><Icon className="h-4 w-4" />{label}</Link>;
}
