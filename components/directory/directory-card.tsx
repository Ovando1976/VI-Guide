    ...
    >
      ...
      <div className="p-5">
        <div className="mb-3 flex items-center justify-between gap-3">
          {item.operatingStatus === "temporarily-closed" || item.operatingStatus === "seasonal" ? (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-100 px-2.5 py-1 text-[9px] font-black uppercase tracking-[.14em] text-amber-900"><Clock3 size={12} /> {item.operatingStatus === "seasonal" ? "Seasonal access" : "Temporarily closed"}</span>
          ) : (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[9px] font-black uppercase tracking-[.14em] text-emerald-800"><BadgeCheck size={12} /> Catalog reviewed</span>
          )}
