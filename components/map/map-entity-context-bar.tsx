...
export function MapEntityContextBar() {
  ...
  return (
    <section
      className="relative z-[1240] mx-auto mb-3 max-w-7xl px-3 sm:px-5"
      aria-label={`Map actions for ${name}`}
    >
        ...
        <div className="p-4 sm:p-5">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-[8px] font-black uppercase tracking-[.17em] text-teal-700">
                Selected · {typeLabel(type)} · {islandLabel(island)}
              </p>
              <h2 className="mt-1 truncate text-xl font-black tracking-[-.035em] text-[#043331]">
                {name}
              </h2>
            ...
            {detailHref ? (
              <Link
                href={detailHref}
                className="hidden min-h-10 shrink-0 items-center gap-1.5 rounded-full bg-[#043331] px-4 text-[8px] font-black uppercase tracking-[.12em] text-white sm:inline-flex"
              >
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
            ...
            <Link
              href={rideHref}
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#f5c451] px-3 text-[9px] font-black uppercase tracking-[.1em] text-[#043331]"
            >
              <Navigation className="h-4 w-4" /> Get official ride price
            </Link>
            <Link
              href={conciergeHref}
              className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#0f766e] px-3 text-[9px] font-black uppercase tracking-[.1em] text-white"
            >
            ...
            {detailHref ? (
              <Link
                href={detailHref}
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-[#043331] px-3 text-[9px] font-black uppercase tracking-[.1em] text-white sm:hidden"
              >
              ...
              <Link
                href="/trips"
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-3 text-[9px] font-black uppercase tracking-[.1em] text-[#043331]"
              >
          ...
          <div className="mt-2 flex items-center justify-between gap-2 border-t border-slate-100 pt-3">
            ...
            <Link
              href={
                `/trips?from=map&place=${encodeURIComponent(id)}&island=${encodeURIComponent(island)}`
              }
              className="inline-flex min-h-9 items-center gap-2 rounded-full px-3 text-[8px] font-black uppercase tracking-[.12em] text-teal-800"
            >
...
function islandLabel(island: "stt" | "stj" | "stx") {
  return island === "stj" ? "St. John" : island === "stx" ? "St. Croix" : "St. Thomas";
}

function typeLabel(type: TerritoryMapPlaceType) {
  return type === "beach" ? "Beach" : type === "stay" ? "Stay" : type === "historic" ? "Historic" : "Place";
}
