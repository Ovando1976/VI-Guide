...
const STYLE_META: Record<MapStyle, { label: string; description: string }> = {
  topographic: {
    label: "Terrain",
    description: "Ridges, elevation, roads, and island context",
  },
  voyager: {
    label: "Island map",
    description: "Clean streets and place context",
  },
  satellite: {
    label: "Satellite",
    description: "Aerial island imagery",
  },
};
...
> = {
  food: {
    label: "Food & drink",
    color: "#e85d2a",
    svg: '<path d="M7 3v7M4 3v4a3 3 0 0 0 6 0V3M7 10v11M17 3v18M17 3c3 2 3 7 0 9"/>',
  },
  attraction: {
    label: "Attraction",
    color: "#0ea5a8",
    svg: '<path d="m12 3 2.6 5.3 5.9.9-4.3 4.2 1 5.9-5.2-2.8-5.2 2.8 1-5.9-4.3-4.2 5.9-.9Z"/>',
  },
  shopping: {
    label: "Shopping",
    color: "#8b5cf6",
    svg: '<path d="M5 8h14l-1 13H6L5 8Z"/><path d="M9 8a3 3 0 0 1 6 0"/>',
  },
  grocery: {
    label: "Grocery",
    color: "#22a447",
    svg: '<path d="M3 4h2l2 11h10l2-7H7"/><circle cx="9" cy="19" r="1.5"/><circle cx="17" cy="19" r="1.5"/>',
  },
  nature: {
    label: "Nature",
    color: "#2f9d67",
    svg: '<path d="M19 4C11 4 6 8 6 14c0 3 2 5 5 5 6 0 8-7 8-15Z"/><path d="M5 21c2-6 6-10 11-13"/>',
  },
  services: {
    label: "Services",
    color: "#64748b",
    svg: '<path d="M12 3v18M3 12h18"/>',
  },
  transport: {
    label: "Transport",
    color: "#d59a16",
    svg: '<path d="M4 16h16M6 16l1-8h10l1 8M8 8l1-3h6l1 3"/><circle cx="8" cy="18" r="2"/><circle cx="16" cy="18" r="2"/>',
  },
  nightlife: {
    label: "Nightlife",
    color: "#db3f83",
    svg: '<path d="M9 18V5l10-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="16" cy="16" r="3"/>',
  },
  general: {
    label: "General place",
    color: "#f97316",
    svg: '<path d="M12 21s7-5.2 7-12a7 7 0 1 0-14 0c0 6.8 7 12 7 12Z"/><circle cx="12" cy="9" r="2.5"/>',
  },
  beach: {
    label: "Beach",
    color: "#06b6d4",
    svg: '<path d="M3 8c2.2 0 2.2 1.6 4.4 1.6S9.6 8 11.8 8s2.2 1.6 4.4 1.6S18.4 8 20.6 8M3 13c2.2 0 2.2 1.6 4.4 1.6S9.6 13 11.8 13s2.2 1.6 4.4 1.6 2.2-1.6 4.4-1.6M3 18c2.2 0 2.2 1.6 4.4 1.6S9.6 18 11.8 18s2.2 1.6 4.4 1.6 2.2-1.6 4.4-1.6"/>',
  },
  historic: {
    label: "Historic site",
    color: "#c58b16",
    svg: '<path d="M3 9h18M5 9V7l7-4 7 4v2M6 9v9M10 9v9M14 9v9M18 9v9M4 18h16M3 21h18"/>',
  },
  stay: {
    label: "Stay",
    color: "#2563eb",
    svg: '<path d="M4 19v-8h16v8M4 15h16M7 11V7h5a3 3 0 0 1 3 3v1M4 19v2M20 19v2"/>',
  },
};
...
export function EstateMap({
  ...
  useEffect(() => {
    ...
    fetch(`/api/restaurants/live?island=${island}`, { signal: controller.signal })
      .then(async (response) => {
        ...
        if (!response.ok) {
          throw new Error(
            payload.error ?? `Restaurant discovery failed (${response.status})`,
          );
        }
  ...
  const mergedPlaces = useMemo(
    () => dedupePlaces([...places, ...livePlaces]),
    [livePlaces, places],
  );
  ...
  const visiblePlaces = useMemo(() => {
    ...
    return placeFilter === "all"
      ? placesByType.place
      : placesByType.place.filter(
          (place) => normalizeSemanticCategory(place) === placeFilter,
        );
    ...
    <div className="premium-territory-map relative overflow-hidden rounded-[28px] border border-[#b9d7d2] bg-[#dcefeb] shadow-[0_24px_70px_rgba(18,49,47,.16)]">
      <div className="pointer-events-none absolute inset-x-0 top-0 z-[1000] flex items-start justify-between gap-3 bg-[linear-gradient(180deg,rgba(7,41,45,.74),rgba(7,41,45,.22),transparent)] px-4 py-4 md:px-5">
        ...
        <div className="pointer-events-auto flex flex-wrap justify-end gap-2">
          ...
          {activeLens === "places" ? (
            <span
              className={`rounded-full border px-3 py-2 text-[9px] font-black uppercase tracking-[.12em] backdrop-blur-xl ${
                liveStatus === "ready"
                  ? "border-emerald-200/60 bg-emerald-400/20 text-white"
                  : liveStatus === "partial"
                    ? "border-amber-200/60 bg-amber-400/20 text-white"
                    : liveStatus === "error"
                      ? "border-rose-200/60 bg-rose-400/20 text-white"
            ...
            >
              {liveStatus === "loading"
                ? "Loading dining"
                : liveStatus === "ready"
                  ? `${livePlaces.length} live dining`
                  : liveStatus === "partial"
                    ? `${livePlaces.length} dining · partial`
                    : liveStatus === "error"
                      ? "Live dining unavailable"
                      : "Dining standby"}
            </span>
          ) : null}
          <MapToggle
      ...
      <div className="absolute right-4 top-[94px] z-[1050] flex flex-col gap-2 sm:flex-row">
        ...
        <button
          ...
          className="rounded-xl border border-white/35 bg-white/94 px-3 py-2 text-[9px] font-black uppercase tracking-[.1em] text-[#12312f] shadow-xl backdrop-blur-xl"
        >
          Full island
        </button>
      </div>
          ...
          {FILTER_ORDER.map((filter) => {
            ...
            return (
              <button
                ...
                className={`flex shrink-0 items-center gap-2 rounded-xl px-3 py-2 text-[10px] font-black transition ${
                  placeFilter === filter
              ...
              >
                ...
                {filter === "all" ? "All places" : meta?.label}{" "}
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[1000] flex justify-center bg-[linear-gradient(0deg,rgba(7,41,45,.34),transparent)] px-4 pb-4 pt-10">
        <div className="pointer-events-auto flex max-w-full gap-1.5 overflow-x-auto rounded-full border border-white/55 bg-white/94 p-1.5 shadow-xl backdrop-blur-xl [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {(
            [
              ["places", "Places", placesByType.place.length, "#f97316"],
              ["beaches", "Beaches", placesByType.beach.length, "#06b6d4"],
              ["historic", "Historic", placesByType.historic.length, "#c58b16"],
              ["stays", "Stays", placesByType.stay.length, "#2563eb"],
            ] as const
          ).map(([lens, label, count, color]) => (
            <button
              key={lens}
              type="button"
      ...
      {selectedPlace ? (
        <div className="absolute bottom-24 left-4 right-4 z-[1100] md:left-5 md:right-auto md:w-[410px]">
          <article className="overflow-hidden rounded-[24px] border border-[#cfe2de] bg-white/96 shadow-[0_24px_70px_rgba(18,49,47,.24)] backdrop-blur-xl">
            <div className="flex items-start justify-between gap-4 p-4 sm:p-5">
              <div className="min-w-0">
                <div
                  ...
                  style={{ color: CATEGORY_META[selectedCategory(selectedPlace)].color }}
                >
                  {CATEGORY_META[selectedCategory(selectedPlace)].label}
                </div>
                ...
                {isFiniteNumber(selectedPlace.rating) ? (
                  <p className="mt-1 text-xs font-black text-[#8a6507]">
              ...
              <button
                ...
                className="min-h-10 rounded-full bg-[#edf7f4] px-3 py-2 text-xs font-black text-[#12312f]"
              >
                Close
              </button>
            </div>
            <div className="grid grid-cols-2 gap-2 border-t border-[#dce8e5] p-3">
              <button
                ...
                className="min-h-11 rounded-xl bg-[#0f766e] px-3 py-2.5 text-xs font-black text-white disabled:opacity-40"
              >
                Plan a ride
              </button>
              <button
                ...
                className="min-h-11 rounded-xl border border-[#e8c66d] bg-[#fff7dc] px-3 py-2.5 text-xs font-black text-[#6b4b00]"
              >
                Save to trip
              </button>
              <Link
                href="/planner"
                onClick={() => savePlaceToTrip(selectedPlace, island)}
                className="flex min-h-11 items-center justify-center rounded-xl bg-[#12312f] px-3 py-2.5 text-center text-xs font-black text-white"
              >
                Plan my day
              </Link>
              <Link
                href={conciergeHref(selectedPlace)}
                className="flex min-h-11 items-center justify-center rounded-xl bg-[#f5c451] px-3 py-2.5 text-center text-xs font-black text-[#12312f]"
              >
                Ask Concierge
              </Link>
              <Link
                href={placeDetailHref(selectedPlace)}
                className="flex min-h-11 items-center justify-center rounded-xl border border-[#cfe2de] bg-[#f6fbf9] px-3 py-2.5 text-center text-xs font-black text-[#12312f]"
              >
                View details
              </Link>
              <button
                ...
                className="min-h-11 rounded-xl border border-[#cfe2de] bg-[#f6fbf9] px-3 py-2.5 text-xs font-black text-[#12312f] disabled:opacity-40"
              >
                Explore area
              </button>
            </div>
...
function SmartPlaceMarkers({
  ...
  onSelect: (key: string, place: PositionedPlace, index: number) => void;
}) {
  const map = useMap();
  ...
  const groups = useMemo(
    () => groupPlaceMarkers(places, zoom, selectedPlaceId),
    ...
    <>
      {groups.map((group) => {
        if (group.items.length === 1) {
          ...
          const selected = selectedPlaceId === item.key;
          return (
            <Marker
              key={item.key}
              pane="pins"
              position={[item.place.lat, item.place.lng]}
              icon={makeSemanticIcon(item.category, selected)}
              eventHandlers={{
                click: () => onSelect(item.key, item.place, item.index),
              }}
            >
              <Tooltip direction="top" offset={[0, -22]} opacity={1}>
                <div className="min-w-[130px]">
                  ...
                  <div className="mt-0.5 text-xs font-black text-[#12312f]">
                    {item.label}
                  </div>
                </div>
              </Tooltip>
        ...
        return (
          <Marker
            ...
            icon={makeClusterIcon(group.items.length, group.color)}
            eventHandlers={{
              click: () =>
                map.flyTo(
                  [group.lat, group.lng],
                  Math.min(17, Math.max(14, zoom + 2)),
          ...
          >
            <Tooltip direction="top" offset={[0, -24]} opacity={1}>
              <div className="min-w-[135px] text-center">
                <div className="text-xs font-black text-[#12312f]">
                  {group.items.length} nearby places
                </div>
                <div className="mt-1 text-[9px] font-bold uppercase tracking-[.12em] text-[#657572]">
                  Tap to zoom in
                </div>
              </div>
            </Tooltip>
...
function groupPlaceMarkers(
  places: PositionedPlace[],
  zoom: number,
  selectedPlaceId: string | null,
): PlaceMarkerGroup[] {
  const items: PlaceMarkerItem[] = places.map((place, index) => {
    ...
    const category: SemanticCategory =
      broadType === "beach"
        ...
        : broadType === "historic"
          ...
          : broadType === "stay"
            ? "stay"
            : normalizeSemanticCategory(place);
    return {
      place,
      index,
      key: placeIdentity(place, index),
      category,
      label: place.name || place.title || `Place ${index + 1}`,
    };
  });

  const cellSize = markerClusterCellSize(zoom);
  if (cellSize <= 0) {
  ...
  const buckets = new Map<string, PlaceMarkerItem[]>();
  for (const item of items) {
    const bucketKey =
      item.key === selectedPlaceId
        ? `selected:${item.key}`
        : `${Math.floor(item.place.lat / cellSize)}:${Math.floor(
            item.place.lng / cellSize,
          )}`;
  ...
  return [...buckets.entries()].map(([bucketKey, bucket]) => {
    const lat =
      bucket.reduce((sum, item) => sum + item.place.lat, 0) / bucket.length;
    const lng =
      bucket.reduce((sum, item) => sum + item.place.lng, 0) / bucket.length;
    return {
      key: bucket.length === 1 ? `place:${bucket[0].key}` : `cluster:${bucketKey}`,
      lat,
      lng,
      color: CATEGORY_META[bucket[0].category].color,
      items: bucket,
    };
  });
}

function markerClusterCellSize(zoom: number) {
  if (zoom <= 10) return 0.055;
  if (zoom <= 11) return 0.032;
...
function BaseMap({ style }: { style: MapStyle }) {
  ...
  return (
    <TileLayer
      attribution="Map © Esri, HERE, Garmin, FAO, NOAA, USGS"
...
function ViewportController({
  ...
  const map = useMap();
  useEffect(() => {
    map.fitBounds(ISLAND_VIEW[island].bounds, {
      padding: [34, 34],
      animate: true,
    });
...
function normalizeSemanticCategory(
  place: PlaceRecord,
): Exclude<SemanticCategory, "beach" | "historic" | "stay"> {
  const text = `${place.category ?? ""} ${place.type ?? ""} ${place.name ?? ""} ${
    place.title ?? ""
...
function makeSemanticIcon(category: SemanticCategory, selected: boolean) {
  ...
  return L.divIcon({
    ...
    html: `<div class="vi-semantic-marker__halo${
      selected ? " is-selected" : ""
    }" style="--marker-color:${meta.color};width:${size}px;height:${size}px"><div class="vi-semantic-marker__face"><svg viewBox="0 0 24 24" aria-hidden="true">${meta.svg}</svg></div><div class="vi-semantic-marker__tip"></div></div>`,
...
function makeClusterIcon(count: number, color: string) {
  const size = count > 20 ? 52 : count > 9 ? 48 : 44;
  return L.divIcon({
    className: "vi-place-cluster",
    html: `<div style="width:${size}px;height:${size}px;border-radius:9999px;display:grid;place-items:center;background:${color};color:#fff;border:3px solid rgba(255,255,255,.96);outline:5px solid rgba(255,255,255,.24);box-shadow:0 10px 28px rgba(18,49,47,.34);font-weight:900;font-size:${count > 99 ? 12 : 14}px;letter-spacing:-.02em">${count}</div>`,
    iconSize: [size, size],
...
function conciergeHref(place: PlaceRecord) {
  const name = place.name || place.title || "this place";
  const location = place.location ? ` in ${place.location}` : "";
  const prompt = `I'm looking at ${name}${location} on the Living Map. Tell me why it's worth visiting, the best time to go, how it fits into my day, and any transport considerations.`;
...
function dedupePlaces(items: PlaceRecord[]): PlaceRecord[] {
  const records = new Map<string, PlaceRecord>();
  for (const item of items) {
    const id = String(
      (item as PlaceRecord & { googlePlaceId?: string }).googlePlaceId ??
        item.id ??
        "",
    );
    ...
    if (!records.has(key) || String(item.id ?? "").startsWith("live-restaurant:")) {
...
function getEstateCenter(estate: EstateRecord): LatLngExpression {
  ...
  const points = getEstatePolygonRings(estate).flat().map(toTuple);
