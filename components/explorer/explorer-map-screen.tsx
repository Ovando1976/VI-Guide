...
import type { TripItemKind } from "@/components/trip-planner/trip-types";
import {
  addTripItem,
  readTrip,
  subscribeToTrip,
...
export function ExplorerMapScreen() {
  ...
  useEffect(() => {
    const syncTrip = (items = readTrip()) => {
      setSavedTripIds(new Set(items.map((item) => item.id)));
    };
    syncTrip();
    return subscribeToTrip(syncTrip);
  }, []);
  ...
  const handlePrepareNamedRoute = useCallback(
    (pickupName: string, destinationName: string) => {
      const normalize = (value: string) =>
        value.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
      const findEntity = (name: string) => {
        const target = normalize(name);
        return allIslandEntities.find((entity) => normalize(entity.title) === target) ?? null;
      };
      const nearestEstate = (name: string) => {
        ...
        if (entity?.position) {
          return islandEstates
            .map((estate) => ({
              estate,
              distance:
                Math.pow(estate.internalPoint.lat - entity.position!.lat, 2) +
                Math.pow(estate.internalPoint.lng - entity.position!.lng, 2),
            }))
            .sort((a, b) => a.distance - b.distance)[0]?.estate ?? null;
        }
        return findEstateByMatchers(islandEstates, [name]);
      };

      const pickupEstate = nearestEstate(pickupName);
      const destinationEstate = nearestEstate(destinationName);
      if (!pickupEstate || !destinationEstate || pickupEstate.geoid === destinationEstate.geoid) return;

      setPickup(pickupEstate.geoid);
      setDestination(destinationEstate.geoid);
      const params = new URLSearchParams({
        island,
        from: pickupEstate.geoid,
        to: destinationEstate.geoid,
    ...
    [allIslandEntities, island, islandEstates, luggage, mode, passengers, router, setDestination, setPickup],
  );
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_right,rgba(14,116,144,0.12),transparent_30%),linear-gradient(180deg,#041018_0%,#07131a_45%,#09161f_100%)] text-white">
      ...
      <ViConcierge
        ...
        onPrepareNamedRoute={handlePrepareNamedRoute}
        placement="right"
