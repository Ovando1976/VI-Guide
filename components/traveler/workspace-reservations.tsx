        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-[10px] font-bold uppercase tracking-[.13em] text-slate-400">
              {directBookingCount} ready to book · {reservableStops.length} bookable stops
            </p>
          </div>
        </div>
        <Link
      ...
      {reservableStops.length ? (
        <div className="mt-5 space-y-3">
          {reservableStops.slice(0, 5).map((stop) => (
            <ReservationStop key={stop.id} journey={journey} stop={stop} />
          ))}
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-[#fbfaf6] p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-[9px] font-black uppercase tracking-[.13em] text-slate-400">
            {bookingHref ? "Booking available" : "Booking assistance needed"}
          </p>
        </div>
      </div>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        <Link
          href={bookingHref || conciergeHref}
          className={`inline-flex min-h-10 items-center justify-center gap-2 rounded-xl px-3 text-[9px] font-black uppercase tracking-[.13em] ${
            bookingHref
              ? "bg-[#043331] text-white"
              : "bg-[#f5b942] text-[#043331]"
          }`}
        >
          {bookingHref ? <ExternalLink className="h-3.5 w-3.5" /> : <Sparkles className="h-3.5 w-3.5" />}
          {bookingHref ? "Book or manage" : "Ask Concierge"}
        </Link>
        <Link
          href={stop.href || stop.mapHref || "/workspace"}
