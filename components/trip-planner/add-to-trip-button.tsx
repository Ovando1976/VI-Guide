...
export function AddToTripButton({ item, className = "", disabled = false, disabledReason = "Currently unavailable" }: Props) {
  ...
  useEffect(() => {
    const sync = (items = readTrip()) => {
      setSaved(items.some((entry) => tripItemKey(entry) === key));
  ...
  return (
    <div className={`grid gap-2 sm:grid-cols-2 ${className}`}>
      ...
      >
        ...
        {disabled ? disabledReason : saved ? "Added to trip" : "Add to trip"}
      </button>
      <Link
        href="/plan"
