...
import { buildTripLegs, tripReadiness } from "./trip-integration";
...
export function TripPlannerScreen() {
  ...
  const legs = useMemo(() => buildTripLegs(items, profile), [items, profile]);
  const readiness = useMemo(() => tripReadiness(items, legs), [items, legs]);
  const conciergePrompt = useMemo(() => {
    ...
    const text = `Review and improve my ${days}-day USVI itinerary starting ${profile.startDate || "on dates not chosen yet"} for ${travelers} traveler${travelers === 1 ? "" : "s"} with ${profile.luggage} bag${profile.luggage === 1 ? "" : "s"}: ${stops || "No stops selected yet"}. Keep island transfers realistic, group nearby stops, prepare ${profile.mobilityPreference} ground transportation between stops, suggest meal breaks, identify conflicts, and give me a practical day-by-day plan. Prepare changes for my confirmation and never remove a saved stop without approval.`;
    return `/map?concierge=open&prompt=${encodeURIComponent(text)}`;
  }, [days, profile, scheduled]);
  ...
  function updateProfile(patch: Partial<TripProfile>) {
    setProfile((current) => {
      const next = { ...current, ...patch };
      writeTripProfile(next);
      return next;
    });
  }
      ...
      <div className="mx-auto max-w-7xl">
        <section className="overflow-hidden rounded-[34px] bg-[linear-gradient(135deg,#043331,#0f766e)] p-7 text-white shadow-[0_28px_70px_rgba(4,51,49,.22)] sm:p-10">
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            ...
            <Link href="/trips/operations" className="inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/10 px-5 py-3 text-[10px] font-black uppercase tracking-[.17em]"><Navigation className="h-4 w-4" /> Ride status</Link>
          </div>
          <div className="mt-7 grid gap-3 border-t border-white/15 pt-6 sm:grid-cols-2 lg:grid-cols-5">
            <TripField label="Start date">
              <input type="date" value={profile.startDate ?? ""} onChange={(event) => updateProfile({ startDate: event.target.value || null })} className="trip-profile-input" />
            </TripField>
            <TripField label="Adults"><input type="number" min={1} max={12} value={profile.adults} onChange={(event) => updateProfile({ adults: Math.max(1, Number(event.target.value) || 1) })} className="trip-profile-input" /></TripField>
            <TripField label="Children"><input type="number" min={0} max={12} value={profile.children} onChange={(event) => updateProfile({ children: Math.max(0, Number(event.target.value) || 0) })} className="trip-profile-input" /></TripField>
            <TripField label="Luggage"><input type="number" min={0} max={24} value={profile.luggage} onChange={(event) => updateProfile({ luggage: Math.max(0, Number(event.target.value) || 0) })} className="trip-profile-input" /></TripField>
            <TripField label="Ride preference"><select value={profile.mobilityPreference} onChange={(event) => updateProfile({ mobilityPreference: event.target.value as TripProfile["mobilityPreference"] })} className="trip-profile-input"><option value="standard">Standard</option><option value="shared">Shared</option><option value="premium">Premium</option></select></TripField>
          </div>
        </section>
              ...
              return (
                <section key={day} className="overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-sm">
                  ...
                  <div className="space-y-3 p-4">
                    ...
                    <DayConnections legs={legs.filter((leg) => leg.day === day)} />
                  </div>
                </section>
              );
            }) : <EmptyTrip />}
          ...
          <aside className="space-y-4 lg:sticky lg:top-6 lg:self-start">
            <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 text-center">
                <Snapshot label="Taxi legs" value={readiness.taxiLegs} />
                <Snapshot label="Transfers" value={readiness.transfers} />
              </div>
            </div>
            {items.length ? <div className={`rounded-[28px] border p-5 ${readiness.ready ? "border-emerald-200 bg-emerald-50" : "border-cyan-200 bg-cyan-50"}`}><div className="text-[10px] font-black uppercase tracking-[.18em]">{readiness.ready ? "Plan connected" : "Needs review"}</div><p className="mt-2 text-xs font-semibold leading-5">{readiness.ready ? "Every same-island connection is ready for transportation review." : `${readiness.transfers} inter-island transfer and ${readiness.unresolvedLocations} location connection require planning.`}</p></div> : null}
...
function TripField({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="rounded-2xl border border-white/15 bg-white/10 px-3 py-2 text-[9px] font-black uppercase tracking-[.14em] text-white/60 [&_input]:mt-1 [&_input]:w-full [&_input]:bg-transparent [&_input]:text-sm [&_input]:font-bold [&_input]:text-white [&_input]:outline-none [&_select]:mt-1 [&_select]:w-full [&_select]:bg-transparent [&_select]:text-sm [&_select]:font-bold [&_select]:text-white [&_select]:outline-none [&_option]:text-[#043331]">{label}{children}</label>;
}

function DayConnections({ legs }: { legs: TripLeg[] }) {
  if (!legs.length) return null;
  return <div className="mt-4 border-t border-slate-100 pt-4"><div className="mb-2 text-[9px] font-black uppercase tracking-[.18em] text-slate-400">Connections</div><div className="grid gap-2">{legs.map((leg) => {
    const Icon = leg.kind === "flight" ? Plane : leg.kind === "ferry" ? Ship : Navigation;
    return <Link key={leg.id} href={leg.href} className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-[#f8f4ea] p-3 transition hover:border-teal-600"><div className="flex min-w-0 items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-teal-800"><Icon className="h-4 w-4" /></span><span className="min-w-0"><span className="block truncate text-xs font-black">{leg.from.name} → {leg.to.name}</span><span className="mt-0.5 block text-[9px] font-bold uppercase tracking-[.12em] text-slate-500">{leg.label}</span></span></div><span className={`shrink-0 rounded-full px-2 py-1 text-[8px] font-black uppercase ${leg.status === "ready_for_review" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}>{leg.status === "ready_for_review" ? "Review ride" : "Ask AI"}</span></Link>;
  })}</div></div>;
}

function Snapshot({ label, value }: { label: string; value: number }) {
  return <div className="rounded-xl bg-[#f8f4ea] px-2 py-3"><div className="text-lg font-black">{value}</div><div className="text-[8px] font-black uppercase tracking-[.12em] text-slate-400">{label}</div></div>;
}
