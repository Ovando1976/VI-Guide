...
export function readTripProfile(): TripProfile {
  if (typeof window === "undefined") return DEFAULT_PROFILE;
  try {
    const value = JSON.parse(
      localStorage.getItem(TRIP_PROFILE_STORAGE_KEY) ?? "{}",
    ...
    return {
      startDate:
        typeof value.startDate === "string" && /^\d{4}-\d{2}-\d{2}$/.test(value.startDate)
          ? value.startDate
          : null,
      adults: clampCount(value.adults, 1, 12, 1),
      children: clampCount(value.children, 0, 12, 0),
      luggage: clampCount(value.luggage, 0, 24, 0),
      mobilityPreference: ["standard", "shared", "premium"].includes(
...
export function writeTripProfile(profile: TripProfile) {
  if (typeof window === "undefined") return;
  localStorage.setItem(TRIP_PROFILE_STORAGE_KEY, JSON.stringify(profile));
  window.dispatchEvent(new CustomEvent(TRIP_UPDATED_EVENT));
}
...
function clampCount(value: unknown, minimum: number, maximum: number, fallback: number) {
