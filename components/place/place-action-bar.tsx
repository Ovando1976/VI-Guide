...
export function PlaceActionBar({ name, island, mapHref, rideHref, website, className = "" }: Props) {
  ...
  return (
    <section className={`rounded-[28px] border border-slate-200 bg-white p-4 shadow-sm ${className}`} aria-label={`Actions for ${name}`}>
...
function Action({ href, icon: Icon, label, accent = false, external = false }: { href: string; icon: typeof Map; label: string; accent?: boolean; external?: boolean }) {
  const classes = `inline-flex min-h-12 items-center justify-center gap-2 rounded-full px-5 text-[10px] font-black uppercase tracking-[.16em] transition hover:-translate-y-0.5 ${
    accent ? "bg-[#f5b942] text-[#043331] hover:bg-[#ffca55]" : "border border-slate-200 bg-[#f8f4ea] text-[#043331] hover:border-[#0f766e] hover:bg-white"
  }`;

  if (external) {
    return <a href={href} target="_blank" rel="noreferrer" className={classes}><Icon className="h-4 w-4" />{label}</a>;
  }

  return <Link href={href} className={classes}><Icon className="h-4 w-4" />{label}</Link>;
}
