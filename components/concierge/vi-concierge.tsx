...
import {
  ...
  tripItemKey,
  writeTrip,
} from "@/components/trip-planner/trip-store";
...
type Props = {
  ...
  onPrepareNamedRoute?: (pickupName: string, destinationName: string) => void;
...
const STARTER_PROMPTS = [
  ...
  "Check fishing rules and seasonal closures",
] as const;
...
export function ViConcierge({
  ...
  const [undoSnapshot, setUndoSnapshot] = useState<TripItem[] | null>(null);
  ...
  useEffect(() => {
    setTripItems(readTrip());
    return subscribeToTrip(setTripItems);
  }, []);

  useEffect(() => {
    if (
      !requestedPrompt || !clientId || !sessionId || !messages.length ||
    ...
    // sendMessage intentionally runs once for each distinct URL prompt.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [clientId, messages.length, requestedPrompt, sessionId]);
  ...
  async function sendMessage(messageOverride?: string) {
    ...
    try {
      ...
      const automaticRoute = payload.actions.find(
        (action) => action.type === "prepare_mobility" && !action.requiresApproval,
      );
      if (automaticRoute) {
        executeAction(automaticRoute);
      }
    } catch (requestError) {
  ...
  function executeAction(action: ConciergeAction, approved = false) {
    if (action.requiresApproval && !approved) {
      setPendingAction(action);
      return;
    }

    const before = readTrip();
    let changedTrip = false;
    ...
    switch (action.type) {
      ...
      case "prepare_mobility":
        if (action.pickupName && action.destinationName && onPrepareNamedRoute) {
          onPrepareNamedRoute(action.pickupName, action.destinationName);
        }
        break;

      case "add_trip_item":
        if (action.tripItem) {
          addTripItem(action.tripItem, {
            day: action.day ?? undefined,
            timeOfDay: action.timeOfDay ?? undefined,
          });
          changedTrip = true;
        }
        break;

      case "schedule_trip_item":
        if (action.target && action.day && action.timeOfDay) {
          writeTrip(before.map((item) => tripItemKey(item) === action.target
            ? { ...item, day: action.day!, timeOfDay: action.timeOfDay! }
            : item));
          changedTrip = true;
        }
        break;

      case "remove_trip_item":
        if (action.target) {
          writeTrip(before.filter((item) => tripItemKey(item) !== action.target));
          changedTrip = true;
        }
        break;

      case "optimize_trip":
        writeTrip(optimizeTrip(before, readTripDays()));
        changedTrip = true;
        break;
    }

    if (changedTrip) setUndoSnapshot(before);
      ...
      {open ? (
        ...
        >
          ...
          >
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="block text-xs font-extrabold text-white/85">
                          {executed ? "Applied" : action.requiresApproval ? `Review: ${action.label}` : action.label}
                        </span>
            ...
            {pendingAction ? (
              <div className="ml-9 rounded-2xl border border-amber-300/25 bg-amber-300/[0.08] p-4">
                <div className="text-[9px] font-extrabold uppercase tracking-[0.18em] text-amber-200/70">Confirm plan change</div>
                <div className="mt-2 text-sm font-extrabold text-white">{pendingAction.label}</div>
                <p className="mt-1 text-xs leading-5 text-white/50">{pendingAction.rationale}</p>
                <div className="mt-3 flex gap-2">
                  <button type="button" onClick={() => executeAction(pendingAction, true)} className="rounded-xl bg-amber-300 px-3 py-2 text-[10px] font-extrabold text-[#3d2a00]">Confirm</button>
                  <button type="button" onClick={() => setPendingAction(null)} className="rounded-xl border border-white/10 px-3 py-2 text-[10px] font-extrabold text-white/60">Cancel</button>
                </div>
              </div>
            ) : null}

            {undoSnapshot ? (
              <button type="button" onClick={undoTripChange} className="ml-9 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-[10px] font-extrabold text-white/65 hover:bg-white/[0.08]">Undo last AI plan change</button>
            ) : null}
