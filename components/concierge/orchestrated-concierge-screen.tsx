      ...
      <section className="border-b border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,.18),transparent_42%),linear-gradient(180deg,#082d38_0%,#061f28_100%)]">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            ...
            <Link
              href="/planner"
              className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[.06] px-4 py-3 text-sm font-black text-white/80 hover:bg-white/10"
            >
        ...
        <section className="space-y-5">
          <div className="rounded-[28px] border border-white/10 bg-white/[.035] p-4 shadow-2xl shadow-black/20 sm:p-6">
            <div className="flex items-center gap-3">
              <span className="grid h-11 w-11 place-items-center rounded-2xl bg-cyan-300 text-[#05242c]">
                <Bot size={21} />
              </span>
              <div>
                <h2 className="font-black">Smart Concierge</h2>
                <p className="text-xs text-white/45">Grounded in VI Guide data</p>
              </div>
            </div>
              ...
              {ISLANDS.map((option) => (
                <button
                  ...
                  className={`shrink-0 rounded-full px-4 py-2 text-xs font-black transition ${
                    island === option.value
                      ? "bg-cyan-300 text-[#05242c]"
                      : "border border-white/10 bg-white/[.04] text-white/60"
                  }`}
                >
                  {option.label}
                </button>
            ...
            {!response ? (
              <div className="mt-6 grid gap-2 sm:grid-cols-2">
                {STARTERS.map((prompt) => (
                  <button
                    ...
                    className="rounded-2xl border border-white/10 bg-white/[.035] p-4 text-left text-sm font-bold leading-6 text-white/70 transition hover:border-cyan-300/30 hover:bg-cyan-300/[.06] hover:text-white"
                  >
                    {prompt}
                  </button>
                ))}
              ...
              <div className="mt-6 space-y-5">
                <div className="rounded-[24px] border border-cyan-200/15 bg-cyan-200/[.055] p-5">
                  <div className="flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-[.14em] text-cyan-100/55">
                ...
                {missing.length ? (
                  <div className="rounded-[24px] border border-amber-300/20 bg-amber-300/[.06] p-5">
                    <div className="flex items-center gap-2 text-sm font-black text-amber-100">
                      <CalendarDays size={17} /> Information needed
                    </div>
                    <div className="mt-4 grid gap-3 sm:grid-cols-2">
                      {missing.map((field) => (
                        <label key={field} className="text-xs font-bold text-white/55">
                          ...
                          <input
                            ...
                            className="h-11 w-full rounded-xl border border-white/10 bg-black/20 px-3 text-sm text-white outline-none focus:border-amber-200/40"
                          />
                        </label>
                      ))}
                    </div>
                    <button
                      ...
                      disabled={missing.some((field) => !details[field]?.trim()) || loading}
                      className="mt-4 inline-flex items-center gap-2 rounded-xl bg-amber-200 px-4 py-3 text-xs font-black text-[#3d2b03] disabled:opacity-35"
                    >
                      ...
                      {response.recommendations.slice(0, 6).map((item) => (
                        <article key={item.id} className="rounded-2xl border border-white/10 bg-white/[.035] p-4">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <p className="text-[9px] font-black uppercase tracking-[.14em] text-cyan-200/50">{item.kind}</p>
                              <h4 className="mt-1 font-black">{item.title}</h4>
                            </div>
                            <MapPin size={16} className="shrink-0 text-cyan-200/60" />
                          </div>
                          <p className="mt-2 line-clamp-3 text-xs leading-5 text-white/50">{item.summary}</p>
                          <button
                            type="button"
                            onClick={() => router.push(item.href ?? item.mapHref ?? "/map")}
                            className="mt-3 text-xs font-black text-cyan-200"
                          >
                            View details →
                          </button>
                        </article>
                ...
                {response.actions.length ? (
                  <div>
                    ...
                    <div className="mt-3 grid gap-2">
                      {response.actions
                        .filter((action) => action.type !== "ask_follow_up")
                        .map((action) => {
                            ...
                            <button
                              ...
                              className={`flex items-center gap-3 rounded-2xl border p-4 text-left transition ${
                                awaitingConfirmation
                                  ...
                                  : "border-white/10 bg-white/[.035] hover:border-cyan-300/25"
                              }`}
                            >
                              <span className="grid h-9 w-9 place-items-center rounded-xl bg-cyan-300/10 text-cyan-200">
                                {action.type === "plan_ride" || action.type === "start_booking" ? (
                                  <Navigation size={16} />
                                ) : (
                                  <CheckCircle2 size={16} />
                                )}
                              </span>
                              <span className="min-w-0 flex-1 text-sm font-black text-white/75">
                                {awaitingConfirmation ? `Confirm: ${action.label}` : action.label}
                              </span>
            ...
            {error ? (
              <div className="mt-4 rounded-2xl border border-rose-300/20 bg-rose-300/[.06] p-4 text-sm text-rose-100">
                {error}
              </div>
            ) : null}
            ...
            <form onSubmit={submit} className="mt-6 flex items-end gap-2 border-t border-white/10 pt-5">
              <textarea
                ...
                rows={2}
                maxLength={3000}
                placeholder="Ask VI Guide to plan, search, arrange a ride, or prepare a booking…"
                className="min-h-14 flex-1 resize-none rounded-2xl border border-white/10 bg-white/[.045] px-4 py-3 text-sm text-white outline-none placeholder:text-white/28 focus:border-cyan-300/35"
              />
              <button
                type="submit"
                disabled={!draft.trim() || loading}
                className="grid h-14 w-14 place-items-center rounded-2xl bg-cyan-300 text-[#05242c] disabled:opacity-35"
        ...
        <aside className="space-y-5">
          <section className="rounded-[28px] border border-white/10 bg-white/[.035] p-5">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-[10px] font-black uppercase tracking-[.16em] text-white/35">Workflow state</p>
                <h2 className="mt-1 text-lg font-black">
                  {loading ? "Working" : workflow?.status === "waiting_for_user" ? "Waiting for you" : ready ? "Ready" : "Not started"}
                </h2>
              </div>
              <span className={`grid h-10 w-10 place-items-center rounded-2xl ${ready ? "bg-emerald-300/15 text-emerald-200" : "bg-cyan-300/10 text-cyan-200"}`}>
                {loading ? <Loader2 size={18} className="animate-spin" /> : ready ? <ShieldCheck size={18} /> : <CircleDot size={18} />}
              </span>
            </div>
            ...
            <div className="mt-5 space-y-2">
              {workflow?.trace.map((step) => (
                <div key={`${step.node}-${step.completedAt}`} className="flex gap-3 rounded-2xl border border-white/8 bg-black/10 p-3">
                  <span className={`mt-0.5 h-2.5 w-2.5 shrink-0 rounded-full ${step.status === "completed" ? "bg-emerald-300" : step.status === "waiting" ? "bg-amber-300" : "bg-cyan-300"}`} />
                  <div className="min-w-0">
                    <p className="text-xs font-black capitalize text-white/70">{step.node}</p>
                    <p className="mt-1 text-[11px] leading-5 text-white/38">{step.detail}</p>
                  </div>
                </div>
              )) ?? (
                <p className="rounded-2xl border border-dashed border-white/10 p-4 text-xs leading-5 text-white/35">
                  Submit a request to see classification, capability checks, grounding, planning, validation, and finalization.
                </p>
              )}
            </div>
          </section>
          ...
          <section className="rounded-[28px] border border-white/10 bg-white/[.035] p-5">
            ...
            <div className="mt-3 flex gap-3">
              <ShieldCheck size={19} className="shrink-0 text-emerald-200" />
              <p className="text-xs leading-6 text-white/48">
                The agent can prepare plans and handoffs, but bookings and consequential actions require your explicit confirmation.
              </p>
            </div>
          </section>
        </aside>
