...
export function BookingPanel({
  ...
  useEffect(() => {
    ...
    fetch("/api/bookings/quote", {
      ...
      .then(async (response) => {
        ...
        if (!response.ok) {
          throw new Error(
            payload.error || "We need dispatch to verify the fare for this trip.",
          );
        }
  ...
  const recommendedMode = useMemo<RideMode>(() => {
    const routeText = `${fromEstate?.baseName ?? ""} ${
      toEstate?.baseName ?? ""
    }`.toLowerCase();
  ...
  const pickupTimingLabel = scheduledAt
    ? formatLocalDateTime(scheduledAt)
    : "Next available dispatch";
  const connectionSummary = connectionDeadline
    ? `${connectionKindLabel(connectionKind)} by ${formatLocalDateTime(
        connectionDeadline,
      )}`
  ...
  function advanceToStep(step: 2 | 3 | 4) {
    setActiveStep(step);
    setFurthestStep(
      (current) => Math.max(current, step) as 1 | 2 | 3 | 4,
    );
  }
  ...
  async function requestRide() {
    ...
    if (!acceptedOperatorDisclosure || !acceptedLegal) {
      setResultTone("error");
      setResultMessage(
        "Please accept the passenger disclosures before confirming your ride request.",
      );
      return;
    }
    ...
    try {
      ...
      setResultMessage("Ride request saved. Opening secure payment…");
        ...
        <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-white/70 sm:text-base">
              Choose the route, see the governed fare, confirm the request, then follow driver status in My Trip.
            </p>
          </div>
              ...
              return (
                ...
                >
                  <div className="text-[8px] font-black uppercase">
                    {furthestStep > stepNumber ? "✓" : `0${step}`}
                  </div>
                  <div className="mt-1 text-[10px] font-black uppercase">{label}</div>
                </button>
              );
            })}
          </div>
      ...
      <div className="p-4 sm:p-6 lg:p-7">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-[24px] border border-slate-200 bg-white px-4 py-3">
          <div className="text-xs font-bold text-slate-500">
            {fromEstate?.baseName || "Your pickup"}{" "}
          ...
          {activeStep === 3 ? (
            <Panel>
              ...
              <div className="mt-5 grid gap-4 rounded-[24px] border border-teal-100 bg-teal-50/70 p-4 sm:grid-cols-2">
                <Field label="Pickup time (optional)" icon={Clock3}>
                  <input
                    type="datetime-local"
                    value={scheduledAt}
                  ...
                  />
                  <p className="mt-2 text-xs font-semibold text-teal-900/60">
                    Leave blank for the next available dispatch.
                  </p>
                </Field>
                <details className="group rounded-[18px] border border-teal-100 bg-white/70 p-3 sm:self-start">
                  <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-[9px] font-black uppercase text-teal-800">
                    <span className="flex items-center gap-2">
                      <Anchor className="h-4 w-4" /> Need to catch something? (optional)
                    </span>
                  ...
                  <div className="mt-3 grid grid-cols-[auto_1fr] gap-2">
                    <select
                      ...
                      onChange={(event) =>
                        setConnectionKind(
                          event.target.value as typeof connectionKind,
                        )
                      }
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        setConnectionDeadline(event.target.value)
                      }
                      className="min-w-0 rounded-[18px] border border-teal-100 bg-white px-3 py-3 text-sm font-bold"
                    />
                  </div>
                  <p className="mt-2 text-xs font-semibold text-slate-500">
                    Add a deadline only when dispatch should protect a ferry, flight,
                    cruise, or appointment connection.
                  </p>
                </details>
              </div>
              <div className="mt-5 grid gap-3 sm:grid-cols-2">
                <Field label="Pickup note (optional)" icon={MapPin}>
                  <textarea
                    ...
                    onChange={(event) =>
                      setPickupInstructions(event.target.value.slice(0, 280))
                    }
                    rows={3}
                ...
                <Field label="Drop-off note (optional)" icon={Route}>
                  <textarea
                    ...
                    onChange={(event) =>
                      setDestinationInstructions(event.target.value.slice(0, 280))
                    }
                    rows={3}
              ...
              <p className="mt-3 text-xs font-semibold text-slate-500">
                We use timing details to help dispatch plan your ride. Driver
                assignment is the point when the ride becomes confirmed.
              </p>
              <StepActions
                backLabel="Back"
            ...
            <aside id="trip-review" className="space-y-5">
              <section className="overflow-hidden rounded-[30px] border bg-white">
                <div className="bg-[#043331] p-5 text-white">
                  <div className="text-[9px] font-black uppercase tracking-[.16em] text-[#f5c451]">
                  ...
                  <p className="mt-2 max-w-2xl text-xs font-semibold leading-5 text-white/65">
                    Confirm the request below. Secure payment starts dispatch; an
                    authorized driver accepting the trip is what confirms your ride.
                  </p>
                </div>
                <div className="p-5">
                  ...
                  {fare ? (
                    ...
                    <div className="mt-5 rounded-[22px] border border-amber-200 bg-amber-50 p-4">
                      ...
                      <p className="mt-2 text-xs font-semibold text-amber-800">
                        We will not guess, estimate, or substitute a fare. Confirm is
                        disabled until the governed fare is available.
                      </p>
                    </div>
                  ) : null}
                </div>
  ...
  return (
    <button
      ...
      className={`relative rounded-[24px] border p-4 text-left ${
        active ? "border-[#f5b942] bg-[#fff4d6]" : "border-slate-200 bg-white"
      }`}
    >
      {active ? (
...
function ReviewFact({
  ...
  return (
    <div className="rounded-[20px] border border-slate-200 bg-[#f8f4ea] p-3.5">
      <div className="flex items-center gap-2 text-[8px] font-black uppercase tracking-[.14em] text-teal-700">
        <Icon className="h-4 w-4" /> {label}
      </div>
      <div className="mt-2 text-sm font-black leading-5 text-[#043331]">{value}</div>
    </div>
  );
}
...
function FareReview({
  ...
  onOperatorDisclosureChange: (value: boolean) => void;
  onLegalChange: (value: boolean) => void;
  onRequest: () => void;
}) {
  const disclosuresAccepted = acceptedOperatorDisclosure && acceptedLegal;
  return (
    <>
      <div className="mt-5 flex items-end justify-between gap-4 border-t border-slate-100 pt-5">
        <div>
          <div className="text-[9px] font-black uppercase text-slate-400">
            Governed fare for this trip
          </div>
      ...
      <section className="mt-5 rounded-[22px] border border-teal-200 bg-teal-50 p-4">
        <div className="text-[9px] font-black uppercase tracking-[.14em] text-teal-700">
          What happens when you confirm
        </div>
        <p className="mt-2 text-sm font-semibold leading-6 text-teal-950">
          We save this ride request and open secure card payment. Payment starts
          dispatch matching. The ride becomes confirmed only after an authorized
          driver accepts the trip.
        </p>
        <div className="mt-4 grid gap-2 sm:grid-cols-3">
          <NextStage icon={CreditCard} label="1 · Pay securely" />
          <NextStage icon={Sparkles} label="2 · Dispatch matches" />
          <NextStage icon={ShieldCheck} label="3 · Driver confirms" />
        </div>
      </section>
      ...
      <div className="ride-confirm-dock sticky bottom-[calc(10px+env(safe-area-inset-bottom))] z-20 -mx-2 mt-6 rounded-[24px] border border-[#e4c35f] bg-[#fff9e7]/95 p-3 shadow-[0_18px_50px_rgba(4,51,49,.22)] backdrop-blur-xl sm:static sm:mx-0 sm:p-4 sm:shadow-none">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <div className="text-[8px] font-black uppercase tracking-[.15em] text-[#8a6507]">
              {canRequest
                ? "Ready to confirm"
                : !pilotActive
                  ? "Preview only"
                  : "Complete the two checks above"}
            </div>
            <div className="mt-1 text-lg font-black text-[#043331]">
              ${fare.total.toFixed(2)} official fare
            </div>
            <p className="mt-1 text-[11px] font-semibold leading-4 text-slate-600">
              Next: secure payment → dispatch matching → driver assignment.
            </p>
          </div>
          <button
            type="button"
            onClick={onRequest}
            disabled={!canRequest}
            className="inline-flex min-h-14 w-full items-center justify-center gap-2 rounded-full bg-[#f5c451] px-5 text-[10px] font-black uppercase text-[#5f3d00] shadow-[0_10px_24px_rgba(245,196,81,.28)] disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto sm:min-w-[250px]"
          >
            {submitting ? (
...
function NextStage({ icon: Icon, label }: { icon: LucideIcon; label: string }) {
  return (
    <div className="flex items-center gap-2 rounded-2xl border border-teal-100 bg-white/80 px-3 py-2.5 text-[9px] font-black uppercase text-teal-900">
...
function formatLocalDateTime(value: string) {
  ...
  if (Number.isNaN(parsed.getTime())) return "Scheduled pickup";
...
function connectionKindLabel(
  value: "flight" | "ferry" | "cruise" | "appointment",
) {
