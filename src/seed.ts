...
import { seedCollection } from "./lib/firestore/seedCollection";

function timestamp() {
  return Date.now();
}

function withTimestamps<T extends Record<string, any>>(record: T): T {
  return {
    ...record,
...
function removeGeometry<T extends Record<string, any>>(record: T): T {
  if (!("geometry" in record)) return record;
...
/**
 * Seed canonical data for VI Navigator / VI Explorer Phase 1.
 */
export async function seedCanonicalData() {
  ...
  const islands: IslandDoc[] = [
    {
      ...
      description:
        "The cosmopolitan heart of the U.S. Virgin Islands, known for its stunning beaches, luxury shopping, and vibrant nightlife.",
      featured: true,
      sortOrder: 1,
    },
    {
      ...
      description:
        "A nature lover's paradise, with over 60% of the island protected as a National Park.",
      featured: true,
      sortOrder: 2,
    },
    {
      ...
      description:
        "The largest of the USVI, offering a rich cultural history, diverse landscapes, and a thriving culinary scene.",
      featured: true,
      sortOrder: 3,
    },
  ];
  ...
  for (const island of islands) {
    await setDoc(doc(db, "islands", island.code), island, { merge: true });
  }
  ...
  for (const area of areas) {
    await setDoc(doc(db, "areas", area.slug), area, { merge: true });
  }
  ...
  const beaches: BeachDoc[] = [
    {
      ...
      description:
        "Consistently rated as one of the world's most beautiful beaches, Magens Bay is a heart-shaped bay with calm, turquoise waters and a mile of white sand.",
      coordinates: { lat: 18.3614, lng: -64.9256 },
    ...
    },
    {
      ...
      areaSlug: "red-hook",
      description:
        "Known for its brilliant blue water and excellent snorkeling right off the shore. Offers great views of St. John and the British Virgin Islands.",
  ...
  const places: PlaceDoc[] = [
    {
      ...
      description:
        "Premium private airport transfers and island tours in luxury SUVs. Featuring digital payments, cold water, and Bluetooth headset systems for clear tour narration.",
    ...
    },
    {
      ...
      areaSlug: "red-hook",
      description:
        "Expert grocery delivery and villa setup for independent travelers. We handle the shopping so you can start your vacation the moment you arrive.",
  ...
  for (const place of places) {
    ...
    await setDoc(doc(db, "places", place.slug), withTimestamps(place), {
      merge: true,
    });
  }
  ...
  const events: EventDoc[] = [
    {
      ...
      areaSlug: "charlotte-amalie",
      description:
        "The biggest cultural celebration of the year! Parades, food, music, and more.",
  ...
  for (const event of events) {
    ...
    await setDoc(doc(db, "events", event.slug), withTimestamps(event), {
      merge: true,
    });
  }
  ...
  await seedCollection("estates", estates);

  const parcels = [
    {
      parcelId: "stt_airport_parcel",
      id: "stt_airport_parcel",
      island: "stt",
      islandCode: "st_thomas",
  ...
  await seedCollection("parcels", parcels);
