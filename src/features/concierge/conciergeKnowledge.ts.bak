...
function uniqueAndRankItems(
  ...
  return items
    ...
    .sort((a, b) => {
      ...
      const aExact = normalizeKnowledgeText(a.item.name || "") === normalizeKnowledgeText(query) ? 1 : 0;
      const bExact = normalizeKnowledgeText(b.item.name || "") === normalizeKnowledgeText(query) ? 1 : 0;
