  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const searchListings = async (args: {
        ...
        results.push(
          ...beachSnapshot.docs.map(
            (doc) => ({ id: doc.id, ...doc.data() } as BeachDoc)
          )
        );
        ...
        results.push(
          ...placeSnapshot.docs.map(
            (doc) => ({ id: doc.id, ...doc.data() } as PlaceDoc)
          )
        );
          ...
          filtered = filtered.filter(
            (l) =>
              ("address" in l &&
                l.address
                  .toLowerCase()
                  .includes(estateFilter!.toLowerCase())) ||
              ("description" in l &&
                l.description
                  .toLowerCase()
                  .includes(estateFilter!.toLowerCase()))
      ...
      const searchEvents = async (args: {
        ...
        const results = snapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() } as EventDoc)
        );
      ...
      const getFavorites = async () => {
        ...
        const favs: Listing[] = [];
        const beachesRef = collection(db, "beaches");
        const qBeaches = query(
          beachesRef,
          where("slug", "in", profile.favorites)
        );
        const beachSnap = await getDocs(qBeaches);
        favs.push(
          ...beachSnap.docs.map((d) => ({ id: d.id, ...d.data() } as BeachDoc))
        );
        ...
        favs.push(
          ...placeSnap.docs.map((d) => ({ id: d.id, ...d.data() } as PlaceDoc))
        );
      ...
      if (contextListing) {
        const category =
          "category" in contextListing ? contextListing.category : "Beach";
        const address =
          "address" in contextListing
            ? contextListing.address
            : "St. Thomas, USVI";
      ...
      if (memories.length > 0) {
        contextPrompt += `Past user preferences/memories: ${memories
            ...
            { googleSearch: {} },
            {
              functionDeclarations: [
                  ...
                  parameters: {
                    ...
                    properties: {
                      key: {
                        type: Type.STRING,
                        description:
                          "The key for the memory (e.g. preferred_island)",
                      },
                      value: {
                        type: Type.STRING,
                ...
                },
                {
                  ...
                  description:
                    "Search community posts and stories from the territory",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      islandCode: {
                ...
                },
                {
                  name: "getTransitInfo",
                  description:
                    "Get real-time transit routes and status for ferries, shuttles, etc.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      islandCode: {
        ...
        for (const call of response.functionCalls) {
          ...
          if (result) {
            const toolResponse = await ai.models.generateContent({
              ...
              config: {
                systemInstruction:
                  "You are the VI Explorer Concierge. Use the provided tool results to give a helpful recommendation. Be specific and friendly.",
              },
            });
      ...
      setMessages((prev) => [
        ...
        {
          role: "model",
          text: "Sorry, I'm having trouble connecting to my local knowledge base right now. Please try again in a moment!",
        },
      ]);
    } finally {
      setIsTyping(false);
      ...
      <div className="p-8 border-b border-stone-100 flex items-center gap-6 bg-white/40 backdrop-blur-3xl sticky top-0 z-20">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <p className="micro-label text-turquoise">
                    ...
                    >
                      <div className="w-14 h-14 rounded-xl overflow-hidden shrink-0 shadow-inner">
                        <img
                          src={
                            listing.coverImage ||
                            `https://picsum.photos/seed/${listing.title}/100/100`
                          }
                          alt=""
                      ...
                      <div className="flex-1 min-w-0 space-y-1">
                        <h4 className="text-base font-serif italic text-ink truncate">
                          {listing.title}
                        </h4>
                        <div className="flex items-center gap-2 text-[9px] text-stone-400 font-bold uppercase tracking-widest">
                          ...
                          <span className="truncate">
                            {(listing as PlaceDoc).address ||
                              (listing as BeachDoc).areaSlug ||
                              "USVI"}
                          </span>
                        </div>
                      </div>
                      <ChevronRight
                        size={16}
                        className="text-stone-200 group-hover:text-turquoise transition-colors"
                      />
                    </div>
                  ))}
                </div>
              ...
              {msg.posts && msg.posts.length > 0 && (
                <div className="space-y-3 pt-2">
                  ...
                  {msg.posts.map((post) => (
                    <div
                      key={post.id}
                      className="bg-sand/50 p-4 rounded-2xl border border-stone-100 space-y-2"
                    >
                      ...
                      <p className="text-xs italic font-serif text-stone-600">
                        "{post.content}"
                      </p>
                    </div>
                  ))}
              ...
              {msg.routes && msg.routes.length > 0 && (
                <div className="space-y-3 pt-2">
                  ...
                  {msg.routes.map((route) => (
                    <div
                      key={route.id}
                      className="bg-sand/50 p-4 rounded-2xl border border-stone-100 flex justify-between items-center"
                    >
                      <div>
      ...
      <div className="p-8 bg-white/40 backdrop-blur-3xl border-t border-stone-100">
        <div className="relative flex items-center gap-6 max-w-4xl mx-auto">
          ...
          >
            <Send
              size={24}
              className="text-turquoise group-hover:rotate-12 transition-transform"
            />
          </button>
        </div>
      </div>
